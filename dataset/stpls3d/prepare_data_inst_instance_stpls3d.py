...
if __name__ == "__main__":
    ...
    out_dir_train = '/root/data/pcdet_data/stpls3d/train/'
    #preparePthFiles(trainFiles, split, trainOutDir, AugTimes=6)
    #preparePthFiles(trainFiles, split, out_dir_train, AugTimes=6)
    ...
    out_dir_val = '/root/data/pcdet_data/stpls3d/val_{0}m'.format(crop_size_val)
    if os.path.exists(out_dir_val):
        print('exist already')
        pdb.set_trace()
    else:
        os.mkdir(out_dir_val)
        preparePthFiles(valFiles, split, out_dir_val, crop_size=crop_size_val)
    #os.makedirs(valOutDir, exist_ok=True)
    #preparePthFiles(valFiles, split, valOutDir, crop_size=250)
    #preparePthFiles(valFiles, split, out_dir_val, crop_size=250)
    #preparePthFiles(valFiles, split, out_dir_val, crop_size=crop_size_val)
