...
from isbnet.pointnet2.pointnet2_utils import FurthestPointSampling_
import numpy as np
import pdb
import time
...
class PointNet(nn.Module):
    def __init__(
        ...
        in_channels = 3#6 if with_coords else 3
        ...
        self.query_num = instance_head_cfg.n_queries
        if not self.semantic_only:
            ...
            self.geo_MLP = GenericMLP(
                input_dim=self.query_num,
                hidden_dims=[self.query_num, self.query_num],
                norm_fn_name="bn1d",
                activation="relu",
                use_conv=True,
                output_dim=1,  # xyz_min, xyz_max
    ...
    def forward_train(
        ...
        **kwargs
        ...
        voxel_coords_float = voxelization(coords_float.contiguous(), p2v_map)
        if epoch > -1:
        ...
        (voxel_batch_offsets, 
         ...
         voxel_instance_labels)=  self.reform_batch(voxel_batch_offsets, voxel_coords_float, voxel_output_feats, 
                          voxel_semantic_labels, voxel_instance_labels)
        pdb.set_trace()
        '''
        ...
        '''
        object_conditions = torch.zeros(len(voxel_coords_float))
        object_conditions = object_conditions == 0
        object_idxs = torch.nonzero(object_conditions).view(-1)
        query_locs, query_feats, query_boxes, query_inds1 = self.point_aggregator1(
            voxel_coords_float,
            voxel_output_feats,
            voxel_box_preds,
            batch_offsets=voxel_batch_offsets,
        ...
        cls_logits, mask_logits, masks, conf_logits, box_preds = self.forward_head(
        ...
        if epoch > -1:
            ...
            losses = self.criterion(batch_inputs, model_outputs)
        # -------------------------------
        locs = self.get_locs_from_mask(dc_coords_float, dc_batch_offsets, masks)
        geo_inputs = self.form_geo_input(cls_logits, conf_logits, locs, box_preds)
        
        geo_feature = self.geo_feature(geo_inputs)
        geo_output = self.geo_MLP(F.relu(geo_feature.permute(0,2,1))).squeeze()
        
        batch_cls_labels_ori = batch_cls_labels.clone()
        batch_cls_labels[batch_cls_labels > 0.5] = 1
        batch_cls_labels[(batch_cls_labels < 0.1) * (batch_cls_labels > 0.05)] = 0
        if epoch > -1:
            geo_loss = F.binary_cross_entropy_with_logits(geo_output, batch_cls_labels)
            losses['geo_loss'] = geo_loss
        
        geo_out = geo_output > 0
        acc = (geo_out == (batch_cls_labels==1)).sum() / (len(batch_cls_labels) + 1e-6)
        pos_samples, neg_samples = (batch_cls_labels==1).sum(), (batch_cls_labels==0).sum()
        pos_acc = (geo_out[batch_cls_labels==1] == 1 ).sum() / ((batch_cls_labels==1).sum() + 1e-6)
        neg_acc = (geo_out[batch_cls_labels==0] == 0 ).sum() / ((batch_cls_labels==0).sum() + 1e-6)
        
        if epoch > -1:
            return self.parse_losses(losses, [acc, pos_samples, neg_samples, pos_acc, neg_acc])
        
        else:    
            return geo_output, cls_logits, conf_logits, locs, pos_acc, neg_acc, batch_cls_labels_ori
    def parse_losses(self, losses, acc):
        ...
        losses['geo_pos'] = acc[1].item()
        losses['geo_neg'] = acc[2].item()
        losses['geo_pos_acc'] = acc[3].item()
        losses['geo_neg_acc'] = acc[4].item()
        losses['geo_acc'] = acc[0].item()
        return loss, losses
    ...
    def forward_test(
        ...
        **kwargs
        ...
        #self.fps.forward([],coords_float,100)
        
        voxel_coords_float = voxelization(coords_float, p2v_map)

        vis_save = False
        if vis_save:
            instance_cls_, instance_box_, centroid_offset_labels, corners_offset_labels = get_instance_info(
            ...
            '''
            #pdb.set_trace()
            feats_np, instance_cls_np = feats.cpu().numpy(), instance_cls_.cpu().numpy()
            ...
            '''
            ret['feats_np'] = feats_np; ret['instance_cls_np']=instance_cls_np
    ...
    def forward_backbone(
        ...
        with context():
            ...
            voxel_input_feats = voxelization(input_feats.contiguous(), p2v_map)
    ...
    def forward_4_parts(self, x):
        ...
        return outs, idxs
    def reform_batch(self, voxel_batch_offsets, voxel_coords_float, voxel_feat, voxel_sem_label, voxel_inst_label):

        valid_voxels = torch.zeros(len(voxel_coords_float)).float().cuda()
        valid_batch = []
        new_batch_offsets = [0]
        offset_num = 0

        for idx in range(len(voxel_batch_offsets)-1):
            start_batch, end_batch = voxel_batch_offsets[idx], voxel_batch_offsets[idx+1]
            num_points = (end_batch - start_batch)  

            if num_points.item() > self.query_num:
                valid_voxels[start_batch: end_batch] = 1
                valid_batch.append(1)
                offset_num += (end_batch.item() - start_batch.item()) 
                new_batch_offsets.append(offset_num)
            else:
                valid_batch.append(0)
        valid_voxels = valid_voxels == 1
        new_batch_offsets = torch.tensor(np.array(new_batch_offsets)).long().cuda()
        valid_batch = torch.tensor(np.array(valid_batch)).long().cuda()

        voxel_coords_float = voxel_coords_float[valid_voxels]
        voxel_feat = voxel_feat[valid_voxels]
        voxel_sem_label = voxel_sem_label[valid_voxels]
        voxel_inst_label = voxel_inst_label[valid_voxels]
        
        return new_batch_offsets, voxel_coords_float, voxel_feat, voxel_sem_label, voxel_inst_label
    def get_locs_from_mask(self, points, batch_offsets, masks):

        locs = []

        for idx in range(len(masks)):
            
            batch_start, batch_end = batch_offsets[idx].item(), batch_offsets[idx+1].item()
            points_batch = points[batch_start: batch_end]

            masks_batch = masks[idx] == 1
            valid_locs = torch.where(masks_batch)
           
            locs_ = []
            for idx_point in range(len(masks_batch)):
                
                valid_points = points_batch[masks_batch[idx_point]]
                if len(valid_points) == 0:
                    locs_.append(torch.tensor([0, 0, 0])[None,:].float().cuda())
                else:
                    center_points = torch.mean(valid_points,0)[None,:]
                    locs_.append(center_points)
            
            locs_ = torch.cat(locs_,0)
            locs.append(locs_)
        return locs
    
    def form_geo_input(self, cls_logits, conf_logits, locs, box_preds):

        geo_inputs = []
        for idx in range(len(locs)):
            
            locs_batch = locs[idx]
            cls_logits_batch = cls_logits[idx]
            conf_logits_batch = conf_logits[idx]

            channel_concat = torch.cat([locs_batch, cls_logits_batch, conf_logits_batch[:,None]],1)
            
            geo_inputs.append(channel_concat[None,:])
        
        geo_inputs = torch.cat(geo_inputs, 0)
        geo_inputs = geo_inputs.permute(0,2,1)
        return geo_inputs
    ...
    def forward_head(
        ...
        if inference:
            return cls_logits[0], mask_logits[0], masks[0], conf_logits[0], queries_box_preds[0]
        return cls_logits, mask_logits, masks, conf_logits, queries_box_preds
    ...
    def mask_heads_forward(
        ...
        #print('{0}, {1}, {2}'.format(relative_coords.shape, relative_boxes.shape, x.shape))
    ...
    def get_instance(
        ...
        #_, idx = torch.topk(cls_scores_flatten, k=min(300, len(cls_scores_flatten)), largest=True) #ScannetV2
        _, idx = torch.topk(cls_scores_flatten, k=min(100, len(cls_scores_flatten)), largest=True) #experimental


        #_, idx = torch.topk(cls_scores_flatten, k=min(20, len(cls_scores_flatten)), largest=True) #experimental
