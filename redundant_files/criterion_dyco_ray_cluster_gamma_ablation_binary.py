...
class Criterion_Dyco_Ray_Cluster_Gamma_Ablation_Binary(nn.Module):
    def __init__(
        self,
        semantic_classes=20,
        instance_classes=18,
        ...
        super(Criterion_Dyco_Ray_Cluster_Gamma_Ablation_Binary, self).__init__()
    ...
    def get_loss_new(self, dc_coords_float_b, inst_label, ray_label, mask_feat_b, dc_proto_features_b, proto_coff_b, mask_logit_pred, ray_pred ):
        ...
        '''
        pdb.set_trace()
        pred_clusters = centerd_coords_dist - torch.tanh(mask_logit_pred.reshape(-1))

        out_pred = ray_dist_with_angles - pred_clusters

        out_pred_mask = out_pred >= 0
        '''
        with torch.no_grad():
        ...
        '''
        '''
        if FP_sample_target.max() >= 0:
            pdb.set_trace()
        if FN_sample_target.min() <= 1:
            pdb.set_trace()
        '''
        ...
        if 'gamma_clustering' in self.activation:
            ...
            if len(FN_sample_target) > 0:
                ...
                #FN_input = mask_logit_pred.reshape(-1)[FN_indice]
