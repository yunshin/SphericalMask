...
from .model_utils import (
    ...
    get_points_mask_from_ray,
    try_vectorize_gamma,
    try_vectorize,
    get_iou,
    try_vectorize_mask2ray
)
import numpy as np
import pdb
import time
class SelfAttention(nn.Module):
    def __init__(self, input_dim):
        ...
        self.inside_mask_head = GenericMLP(
                input_dim=32,
                hidden_dims=[32, 32],
                norm_fn_name="bn1d",
                activation="relu",
    ...
    def forward(self, query, key, value):
        ...
        out = self.inside_mask_head(F.relu(weighted.squeeze()))
        #pdb.set_trace()
        return out
class PolarNet_Dyco_Ray_Cluster_Gamma_Ablation(nn.Module):
    def __init__(
        self,
        ...
        if not self.semantic_only:
            ...
            self.inst_center_head = GenericMLP(
                input_dim=instance_head_cfg.dec_dim,
                hidden_dims=[instance_head_cfg.dec_dim, instance_head_cfg.dec_dim],
                norm_fn_name="bn1d",
                activation="relu",
            ...
            if self.instance_head_cfg.num_dyco_layer == 2:
                out_dim = 32
            else:
                out_dim = 16
            self.inst_inside_mask = GenericMLP(
                input_dim=channels,
                hidden_dims=[32,32,32],#[32, 32],
                norm_fn_name="bn1d",
                activation="relu",
                use_conv=False,
                output_dim=out_dim,  # xyz_min, xyz_max
                #output_dim=1,  # xyz_min, xyz_max
            )
            
            self.inside_mask_head = GenericMLP(
                input_dim=out_dim,
                hidden_dims=[out_dim,out_dim],#[32, 32],
                norm_fn_name="bn1d",
                activation="relu",
                use_conv=False,
                output_dim=out_dim,  # xyz_min, xyz_max
            ...
            #self.inside_mask_head = SelfAttention(32)
            if self.criterion != None:
                self.criterion.set_mask_head(self.inside_mask_head)
            # NOTE dyco
            self.init_dyco()
    ...
    def init_dyco(self):
        ...
        if hasattr(self.instance_head_cfg, 'num_dyco_layer'):
            self.num_dyco_layer = self.instance_head_cfg.num_dyco_layer
        else:
            self.num_dyco_layer = 3
    ...
    def init_dyco_angular(self):
        ...
        #self.angular_feature_dim = 1
        self.angular_feature_dim = self.instance_classes + 1
        ...
        #controller_head = nn.Conv1d(self.instance_head_cfg.dec_dim, 32, kernel_size=1)
        if self.num_dyco_layer == 3:
            controller_head = nn.Conv1d(self.instance_head_cfg.dec_dim, 16, kernel_size=1)
        else:
            controller_head = nn.Conv1d(self.instance_head_cfg.dec_dim, 32, kernel_size=1)
        torch.nn.init.normal_(controller_head.weight, std=0.01)
        torch.nn.init.constant_(controller_head.bias, 0)
    ...
    def get_output_old(self, dc_coords_float, mask_logits, rays_labels, query_locs, dc_mask_features_permute, proto_coff):
        masks_proto = torch.zeros_like(mask_logits) 
        masks_proto[:,:] = -100
        
        closest_gt = torch.cdist(query_locs[0], rays_labels[:,-3:,0])
        closest_gt = torch.argmin(closest_gt, 1)
        ray_corres_gt = rays_labels[closest_gt,:,0]

        all_masks_ray_, all_pred_mask_baseline, final_pred_mask, roi_indice = get_points_mask_from_ray(dc_coords_float, (mask_logits>=0)*1.0, ray_corres_gt.clone(), self.rays_width, self.rays_height,with_pred=True, pred_mask=mask_logits)
        ...
        for mask_idx in range(len(all_masks_ray_)):
            ...
            roi_mask_pred = torch.matmul(dc_mask_features_permute[roi_indice_idx].squeeze(),proto_coff[mask_idx][:,None])
            #roi_mask_pred = dc_mask_features_permute[roi_indice_idx].squeeze()
            masks_proto[mask_idx, roi_indice_idx] = roi_mask_pred.squeeze()
        masks_proto[masks_proto>=0] = 1

        return masks_proto
    
   
    def get_output_new_ray(self, dc_coords_float, mask_logits, rays_labels, query_locs, dc_mask_features_permute, mask_feats, proto_coff, gt_masks, ray_preds):
        masks_proto = torch.zeros_like(mask_logits) 
        masks_proto[:,:] = -100
        

        rays_from_mask = try_vectorize_mask2ray(dc_coords_float,  (mask_logits>=0)*1.0,  self.rays_width, self.rays_height, self.angles_ref)    
        ray_preds[ray_preds<0] = 0
        rays_from_mask[:,:-3] = rays_from_mask[:,:-3] + ray_preds[:,:-3] 
        #if 'ray' in self.activation:
        #    rays_from_mask[:,-3:] = rays_from_mask[:,-3:]*0.2 + ray_preds[:,-3:]*0.8
        
    
        if 'gamma_clustering' in self.activation:
            #rays_from_mask[:,:-3] =  ray_preds[:,:-3] ################## for ray ablation
            
            dist_anchors, angles_num_pred_, roi_locs, centerd_coords_ = try_vectorize_gamma(dc_coords_float, mask_logits, rays_from_mask, self.rays_width, self.rays_height)      
            #dist_anchors, angles_num, roi_locs, _ = try_vectorize(dc_coords_float, mask_logits, rays_from_mask, self.rays_width, self.rays_height) 
            ...
            ray_dist_with_angles = ray_preds[angles_num_with_batch[:,0],angles_num_with_batch[:,1]]

            pred_clusters = centerd_coords_dist - mask_logits.reshape(-1)
            #pred_clusters = centerd_coords_dist - torch.tanh(mask_logits.reshape(-1))
            #pred_clusters = centerd_coords_dist - torch.sigmoid(mask_logits.reshape(-1))
            out_pred = ray_dist_with_angles - pred_clusters
            
            out_pred_mask = (out_pred >= 0)*1.0
            out_pred_mask = out_pred_mask.reshape(mask_logits.shape)
        else:
            dist_anchors, angles_num, roi_locs, _ = try_vectorize(dc_coords_float, mask_logits, rays_from_mask, self.rays_width, self.rays_height) 
        
        
        if '2nd_refine' in self.activation:
            mask_feats = mask_feats.permute(0,2,1)
            feats1 = mask_feats[roi_locs[0],roi_locs[1]].squeeze()
            feats2 = dc_mask_features_permute[roi_locs[1]].squeeze()
        
            feats = feats1 + feats2
            
            
            feats = self.inside_mask_head(F.relu(feats))
            
            roi_pred_total = torch.matmul(feats, proto_coff.T)
            roi_pred_total = roi_pred_total[torch.arange(len(roi_pred_total)).long().cuda(),roi_locs[0]]

            masks_proto[roi_locs[0], roi_locs[1]] = roi_pred_total.squeeze()
        elif '2nd_refine' not in self.activation and 'gamma_clustering' in self.activation:
            masks_1 = torch.zeros_like(masks_proto)
            masks_2 = mask_logits.clone() >= 0
            
            masks_1[roi_locs[0], roi_locs[1]] = 1
            
            masks_merged = masks_1 * masks_2
            masks_proto = masks_1
            #
            masks_proto = out_pred_mask
            masks_proto[masks_proto<=0] = -100
        else:
        
            masks_proto[roi_locs[0], roi_locs[1]] = 1
        masks_proto[masks_proto>=0] = 1
    ...
    def forward_train(
        ...
        **kwargs
        ...
        voxel_batch_idxs = batch_idxs[p2v_map[:, 1].long()]
        ...
        instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, corners_offset_labels_sph, rays_labels, spherical_mask, voxel_semantic_labels = get_instance_info_dyco_ray(
            voxel_coords_float, voxel_batch_idxs, voxel_instance_labels, voxel_semantic_labels, self.rays_width, self.rays_height, self.angles_ref, label_shift=self.label_shift
        )
        ###################
        #time_pre = time.time()
        batch_inputs = dict(
        ...
        if self.use_spp_pool:
            ...
            dc_inst_mask_arr = get_spp_gt_ray_final(
        ...
        #dc_mask_features_permute = self.inst_inside_mask(F.relu(dc_mask_features.detach().squeeze()))#1
        #dc_mask_features_permute = self.inst_inside_mask(F.relu(dc_mask_features.squeeze()))#2
       
        dc_mask_features_permute = self.inst_inside_mask(dc_output_feats.squeeze())
        #dc_mask_features_permute = self.inst_inside_mask(torch.unsqueeze(dc_output_feats, dim=2).permute(2, 1, 0)).permute(2, 1, 0).squeeze()
        cls_logits, mask_logits, conf_logits, box_preds, proto_coff, mask_feats, ray_preds = self.forward_head(
        ...
        model_outputs.update(
            dict(
                ...
                box_preds=box_preds,
                ray_preds=ray_preds,
                proto_coff=proto_coff,
                mask_feats=mask_feats,
                dc_mask_features_angular=dc_mask_features_permute
            )
        )
        #time1 = time.time()
        losses = self.criterion(batch_inputs, model_outputs)
        # -------------------------------
        self.train_iter += 1
        self.criterion.set_train_iter(self.train_iter)
        time2 = time.time()
        #print('polarnet time pre: {0}, time until dc: {1} loss calculation: {2}'.format(time_pre-start_time, time_dc - time_pre,time2-time_dc))
        return self.parse_losses(losses)
    ...
    def parse_losses(self, losses):
        ...
        return loss, losses
    def make_individual_mask(self, inst_label):
        inst_ids = torch.unique(inst_label)
        masks = []
    ...
    def forward_test(
        ...
        **kwargs
        ...
        voxel_batch_idxs = batch_idxs[p2v_map[:, 1].long()]
        voxel_instance_labels = get_cropped_instance_label(voxel_instance_labels)
        #voxel_feats = feats[p2v_map[:, 1].long()]
        voxel_spps = spps[p2v_map[:, 1].long()]
        voxel_spps = torch.unique(voxel_spps, return_inverse=True)[1]
        ...
        gt_masks = self.make_individual_mask(voxel_instance_labels)
        
        times = []
        for chk_idx in range(5):
            start_time = time.time()
            voxel_output_feats, voxel_batch_idxs = self.forward_backbone(
        ...
        gt_masks = gt_masks[instance_cls != -100]
        for i in range(len(n_sample_arr)):
            ...
            for idx_time in range(5):
                ...
                cls_logits, mask_logits, conf_logits, box_preds, proto_coff, mask_feats, ray_preds = forward_head_outputs
                times.append(time.time() - start_time)
            radial_times = np.array(times)
            
            if forward_head_outputs is None:###need to delete
                    break
            pdb.set_trace()
            if type == 0:
                masks_proto = torch.zeros_like(mask_logits) 
                ...
                masks_ = mask_logits >= 0
                if masks_.sum() == 0:
                    masks_[:,0] = 0
                locs0, locs1 = torch.where(masks_)
                feats = dc_mask_features_permute[locs1].squeeze()
                roi_pred_total = torch.matmul(F.relu(feats), proto_coff.T)
                try:
                    roi_pred_total = roi_pred_total[torch.arange(len(roi_pred_total)).long().cuda(),locs0]
                except:
                    pdb.set_trace()
                masks_proto[locs0, locs1] = roi_pred_total.squeeze()
                masks_proto[masks_proto>=0] = 1
                mask_logit = masks_proto[:, voxel_spps]

            elif type == 1:
               
                rays_label_ = rays_labels[instance_cls!= -100]
                times = []
                for idx_time in range(5):
                    start_time = time.time()
                    masks_proto = self.get_output_new_ray(dc_coords_float, mask_logits, rays_label_, query_locs, dc_mask_features_permute, mask_feats, proto_coff, gt_masks, ray_preds)
                    
                    mask_logit = masks_proto[:, voxel_spps]
                    times.append(time.time() - start_time)
                assembly_times = np.array(times)

                
                #masks_proto = self.get_output_old(dc_coords_float, mask_logits, rays_labels, query_locs, dc_mask_features_permute, proto_coff)
                
                
                
                fp = open('runtime.txt','a+')
                num_points = len(coords_float)
                fp.write('{0}|'.format(num_points))
                for runtime_idx in range(len(backbone_times)):
                    if runtime_idx < (len(backbone_times) -1):
                        fp.write('{0},'.format(backbone_times[runtime_idx]))
                    else:
                        fp.write('{0}'.format(backbone_times[runtime_idx]))
                fp.write('|')
                for runtime_idx in range(len(radial_times)):
                    if runtime_idx < (len(backbone_times) -1):
                        fp.write('{0},'.format(radial_times[runtime_idx]))
                    else:
                        fp.write('{0}'.format(radial_times[runtime_idx]))
                fp.write('|')
                for runtime_idx in range(len(assembly_times)):
                    if runtime_idx < (len(backbone_times) -1):
                        fp.write('{0},'.format(assembly_times[runtime_idx]))
                    else:
                        fp.write('{0}'.format(assembly_times[runtime_idx]))    
                fp.write('\n')
                fp.close()
                #pdb.set_trace()
                #backbone_times, radial_times, assembly_times
            elif type == 2:
        ...
        if vis_save:
            ...
            instance_cls_, instance_box_, instance_sphere_, centroid_offset_labels_, pointwise_angular_labels_, corners_offset_labels_, corners_offset_labels_sph_, rays_labels_, spherical_mask_, voxel_semantic_labels_, real_points  = get_instance_info_dyco_ray(
                coords_float, batch_idxs, instance_labels, semantic_labels, self.rays_width, self.rays_height, self.angles_ref, label_shift=self.label_shift, is_vis=True
            )
            
            #save_path = '/root/data/pcdet_data/scannet200/vis/'
            save_path = '/root/data/pcdet_data/scannet_pcdet_data/scannet_val_data_qual/'
            feats_np, instance_cls_np, rays_labels_np = feats.cpu().numpy(), instance_cls_.cpu().numpy(), rays_labels_.cpu().numpy()
            semantic_labels_np, instance_labels_np = semantic_labels.cpu().numpy(), instance_labels.cpu().numpy()
            coords_float_np = coords_float.cpu().numpy()
            instance_sphere_np = instance_sphere_.cpu().numpy()
            query_locs_np = query_locs.cpu().numpy()

            
            np.save('{0}/{1}_rgb.npy'.format(save_path, scan_ids[0]), feats_np)
            np.save('{0}/{1}_cls.npy'.format(save_path, scan_ids[0]), instance_cls_np)
            np.save('{0}/{1}_rays.npy'.format(save_path, scan_ids[0]), rays_labels_np)
            np.save('{0}/{1}_sem_label.npy'.format(save_path, scan_ids[0]), semantic_labels_np)
            np.save('{0}/{1}_inst_label.npy'.format(save_path, scan_ids[0]), instance_labels_np)
            np.save('{0}/{1}_points.npy'.format(save_path, scan_ids[0]), coords_float_np)
            np.save('{0}/{1}_sphere.npy'.format(save_path, scan_ids[0]), instance_sphere_np)
            np.save('{0}/{1}_real_points.npy'.format(save_path, scan_ids[0]), np.array(real_points))
            np.save('{0}/{1}_query_locs.npy'.format(save_path, scan_ids[0]), query_locs_np)
            print('{0}/{1}_inst_label.npy'.format(save_path, scan_ids[0]))
    ...
    def forward_head(
        ...
        ray_offsets_preds = self.inst_ray_head(queries_features).transpose(1, 2)
        center_offsets_preds = self.inst_center_head(queries_features).transpose(1, 2)

        center_offsets_preds = center_offsets_preds + queries_locs
       
        queries_box_preds = box_offsets_preds + queries_locs.repeat(1, 1, 2)

        
        ray_offsets_preds = torch.cat([ray_offsets_preds, center_offsets_preds],2)
        if inference:
        ...
        '''
        controllers_angular = (
            self.inst_mask_head_angular(queries_features.detach().permute(0, 2, 1)[..., None].flatten(0, 1))
            .squeeze(dim=2)
            .reshape(batch_size, n_queries, -1)
        )
        '''
        controllers_angular = (
            self.inst_mask_head_angular(queries_features.permute(0, 2, 1)[..., None].flatten(0, 1))
            .squeeze(dim=2)
            .reshape(batch_size, n_queries, -1)
        )
        
        proto_coff = controllers_angular
        #proto_coff = controllers
        mask_logits = []
        mask_feats = []
        for b in range(batch_size):
            ...
            for i in range(num_chunks):
                ...
                mask_logit_, mask_feat_ = self.mask_heads_forward(
                    mask_feature_b,
                    weights_split,
                    biases_split,
                ...
                mask_feat.append(mask_feat_)
            mask_logit = torch.cat(mask_logit, dim=0)
            mask_feat = torch.cat(mask_feat, dim=0)
        ...
        if inference:
            return cls_logits[0], mask_logits[0], conf_logits[0], queries_box_preds[0], proto_coff[0], mask_feats[0], ray_offsets_preds[0]
        return cls_logits, mask_logits, conf_logits, queries_box_preds, proto_coff, mask_feats, ray_offsets_preds
    ...
    def get_instance(
        ...
        _, idx = torch.topk(cls_scores_flatten, k=min(50, len(cls_scores_flatten)), largest=True)
        #_, idx = torch.topk(cls_scores_flatten, k=min(3500, len(cls_scores_flatten)), largest=True)
        ...
        # NOTE NMS
        masks_final, cls_final, scores_final, boxes_final, _ = nms(
