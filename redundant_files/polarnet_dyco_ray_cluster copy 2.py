...
class PolarNet_Dyco_Ray_Cluster(nn.Module):
    ...
    def forward_train(
        ...
        **kwargs
        ...
        dc_mask_features_permute = self.inst_inside_mask(F.relu(dc_mask_features.squeeze()))
        
        cls_logits, mask_logits, mask_ray_logits, conf_logits, box_preds, rays_pred, proto_coff = self.forward_head(
    ...
    def forward_test(
        ...
        **kwargs
        ...
        for i in range(len(n_sample_arr)):
            ...
            cls_logits, mask_logits, mask_ray_logits, conf_logits, box_preds, rays_pred, proto_coff = forward_head_outputs
            ...
            roi_mask_pred = dc_mask_features_permute[roi_locs[1]].squeeze()
            
            roi_pred_total = torch.matmul(F.relu(dc_mask_features_permute[roi_locs[1]].squeeze()), proto_coff.T)
    ...
    def forward_head(
        ...
        if inference:
            return cls_logits[0], mask_logits[0], mask_ray_logits[0], conf_logits[0], queries_box_preds[0], rays_pred[0], proto_coeff[0] #angular_preds[0]
        return cls_logits, mask_logits, mask_ray_logits, conf_logits, queries_box_preds, rays_pred, proto_coeff#, angular_preds
