...
from .model_utils import (
    ...
    get_batch_offsets,
    get_instance_info,
    get_instance_info_polarnet_exp5,
    get_spp_gt,
    get_subsample_gt,
    nms,
    random_downsample,
...
import pdb
import time
class PolarNet_Direct_EXP5(nn.Module):
    def __init__(
        self,
        is_mask=True,
        model_name='polarnet_direct_exp5',
        channels=32,
        num_blocks=7,
        semantic_only=False,
        semantic_classes=20,
        ...
        if not self.semantic_only:
            ...
            if rays_width == 0:
                ...
                self.inst_box_head = GenericMLP(
                    ...
                    activation="relu",
                    use_conv=True,
                    output_dim=rays_width*rays_height ,  # xyz_min, xyz_max
                )
    ...
    def init_dyco(self):
        ...
        weight_nums = [(self.mask_dim_out + self.rays_width*(self.rays_height) + 3) * self.mask_dim_out, self.mask_dim_out * 1]
    ...
    def refine_samples(self, box_preds, box_gt, spherical_mask):
        ...
        pdb.set_trace()
    def refine_center(self, voxel_box_preds_nms, category_labels, corners_offset_labels_sph, instance_cls, voxel_coord_float, center_refine=False, ray_refine=False):
    ...
    def forward_train(
        ...
        **kwargs
        ...
        instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, corners_offset_labels_sph, rays_labels, spherical_mask, voxel_semantic_labels  = get_instance_info_polarnet_exp5(
        ...
        voxel_box_preds = torch.cat([voxel_box_preds, voxel_centroid_offset.clone()],1)## concating this before adding offset in order to make it relative posiiton during sampling
        ...
        **kwargs
        ...
        if True:#self.is_mask == False:
            ...
            if self.is_analyze:
                ...
                instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, rays_gt_list, voxel_semantic_labels = get_instance_info_polarnet_exp5(
            ...
            instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, corners_offset_labels_sph, rays_gt, spherical_mask, voxel_semantic_labels = get_instance_info_polarnet_exp5(
                voxel_coords_float, voxel_instance_labels, voxel_semantic_labels, self.rays_width, self.rays_height, self.angles_ref, label_shift=self.label_shift
            )
        ...
        for i in range(len(n_sample_arr)):
            ...
            if self.is_mask:
                ...
                pdb.set_trace()
                mask_logit = mask_logits[:, voxel_spps]#voxel spps is the voxel to point conversion. Basically, this means that each mask is predicting mask for all pixel as once.
    ...
    def forward_head(
        ...
        queries_box_preds = box_offsets_center_preds + queries_locs
        queries_box_preds = torch.cat([box_offsets_preds, queries_box_preds],2)
    ...
    def parse_dynamic_params(self, params, out_channels):
        ...
        weight_splits[0] = weight_splits[0].reshape(num_instances, out_channels + self.rays_width*(self.rays_height)+3, out_channels)
    ...
    def mask_heads_forward(
        ...
        if boxes_.shape[1] > (self.rays_width*self.rays_height):
            boxes_ = boxes_[:,:-3]
        relative_boxes = torch.abs( queries_boxes[:,:-3].reshape(-1, 1, self.rays_width*(self.rays_height)) - boxes_.reshape(1, -1, self.rays_width*(self.rays_height)))
        
        #relative_boxes = torch.abs(
        #    queries_boxes_dim.reshape(-1, 1, self.rays_width*(self.rays_height) + 3) - boxes_dim.reshape(1, -1, self.rays_width*(self.rays_height) + 3)
        #)  # N_inst * N_mask * 3
        #relative_boxes = queries_boxes_dim.reshape(-1,1, self.rays_width*(self.rays_height) + 3)
        relative_boxes = relative_boxes.permute(0, 2, 1)
        
        x = torch.cat([relative_coords, relative_boxes, x], dim=1)  # num_inst * (3+c) * N_mask
        
        for i, (weight_dyco, bias_dyco) in enumerate(zip(weights, biases)):
            if i < n_layers - 1:
               
                x = torch.einsum("qab,qan->qbn", weight_dyco, x) + bias_dyco.unsqueeze(-1)
                x = F.relu(x)
            else:
                x = torch.einsum("qab,qan->qbn", weight_dyco, x) # NOTE Empirically, we do not add biases in last dynamic_conv layer
                x = x.squeeze(1)
