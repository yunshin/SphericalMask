    ...
    def single_layer_loss(
        ...
        for b in range(batch_size):
            ...
            if is_aux == False:
                ...
                angular_mask = angular_label_b[:,0] > -100
               
                num_inst_angular = 10
                cos_sim = F.cosine_similarity(angular_pred_b[angular_mask][:num_inst_angular], angular_label_b[angular_mask][:num_inst_angular])
                sim_loss = (1 - cos_sim)
                sim_loss = sim_loss.sum() / angular_mask.sum()

                #angular_loss_l1 = F.l1_loss(angular_pred_b[angular_mask][:num_inst_angular], angular_label_b[angular_mask][:num_inst_angular], reduction="none")
                #angular_loss_l1 = angular_loss_l1.sum() / (angular_mask.sum() + 1e-6)

                pos_angular_samples = cos_sim > 0.8
                pos_angular_samples_total += pos_angular_samples.sum().item()
                num_angular_samples_total += num_inst_angular
                #num_angular_samples_total += angular_mask.sum().item()
