...
def try_vectorize_mask2ray(dc_coords_float, mask_all,  rays_width, rays_height, angles_ref, center_provided=False, centers_=None):
    ...
    if center_provided == False:
        ...
        centers = centers_
    #centers = custom_scatter_mean(all_locs, true_locs[0], pool=True, output_type=torch.float32)
    '''
    centers_manual = []
    valid_idx = []
    for chk_idx in range(len(mask_all)):
        
        locs_idx = dc_coords_float[true_locs[1][true_locs[0] == chk_idx]]
        if len(locs_idx)> 0:
            center_idx = torch.mean(locs_idx,0)
            centers_manual.append(center_idx[None,:])
            valid_idx.append(True)
        else:
            valid_idx.append(False)
    try:
        centers_manual = torch.cat(centers_manual)
        
        valid_idx = torch.tensor(np.array(valid_idx)).cuda()
    
        diff = torch.abs(centers[valid_idx] - centers_manual).sum(1)
    except:
        pdb.set_trace()
    if torch.isnan(diff).any().item() or diff.max().item() > 1e-4:
        pdb.set_trace()
    '''
    ...
    #for debug
    #sph_label, pointwise_angular_gt, ious_upper_bound, rays_gt = get_sph_gt( dc_coords_float[mask_all[-1]==1], centers[-1], rays_width, rays_height, angles_ref, chk_iou=False, all_points=dc_coords_float, inst_loc=None)
...
def try_vectorize_gamma(dc_coords_float, mask_all, rays, rays_width, rays_height, inst_label=None):
    ...
    if False and inst_label != None:
        
        foreground_before = new_pts_vec[:,:,3][inst_label == 1]
        background_before = new_pts_vec[:,:,3][inst_label == 0]

        mod_dist = new_pts_vec[:,:,3] - torch.tanh(mask_all) 

        foreground_after = mod_dist[inst_label == 1]
        background_after = mod_dist[inst_label == 0]

        print('f/b before: {0}/{1} f/b min/max: {2}/{3} f/b after: {4}/{5} f/b min/max:  {6}/{7}'.format(foreground_before.mean(), background_before.mean(),
                                                               foreground_before.min(), background_before.max(),                                                             
                                                               foreground_after.mean(), background_after.mean(),
                                                               foreground_after.min(), background_after.max()))

        minus_indice = torch.where(mod_dist < 0)

    #new_pts_vec[:,:,3] = new_pts_vec[:,:,3] - torch.tanh(mask_all) 
...
def get_sph_gt_ray(inst_points, inst_points_center, rays_width, rays_height, angles_ref, num_range=20,centering=False, chk_iou=True, all_points=None, inst_loc=None):
    ...
    diff = (centered_points - old_pts).sum(1)
    if diff.max() > 1e-4:
        pdb.set_trace()
    new_pts[:,4:] = torch.rad2deg(new_pts[:,4:])
    new_pts[:,5] += 180
    ...
    minus_angle_mask1 = new_pts[:,4] < 0
    
    chk_minus1 = new_pts[minus_angle_mask1,4] < 0
    chk_minus2 = new_pts[minus_angle_mask1,5] < 0
    chk_max1 = new_pts[:,5].max() 
    chk_max2 = new_pts[:,4].max() 
    if chk_minus1.sum() > 0 or chk_minus2.sum() > 0 or chk_max2 > 180 or chk_max1 > 360:
        print('chk_minus found to be > 0')
        pdb.set_trace()
    
    dist_label_anchors, angles_num_each = divide_with_angle_vectorize(new_pts[:,3:], rays_width, rays_height, return_ref=True, cartesian_ref=inst_points)
    
    dists = new_pts[:,3]
    num_rays = rays_height*rays_width
   
   
    
    mask_rays = get_mask_inside_ray(dists, dist_label_anchors, angles_num_each, num_rays, num_range)
    mask_indice_from_ray = get_mask_indice_inside_ray(dists, dist_label_anchors, mask_rays, angles_num_each, num_rays, num_range)
    ...
    if centering == False:
        ...
        sph_points += inst_points_center[None,:]
        if chk_iou:
            return sph_points, pointwise_angular_gt, [iou_pure_ray.item(), iou_angular.item()], dist_label_anchors[:,None], angles_num_each, mask_rays, mask_indice_from_ray
        return sph_points, pointwise_angular_gt, [], dist_label_anchors[:,None], angles_num_each, mask_rays , mask_indice_from_ray
    return dist_label_anchors, angles_num_each
def get_instance_info_polarnet_exp1(coords_float, instance_labels, semantic_labels, rays_width, rays_height, angles_ref, label_shift=2, is_analyze=False):
    instance_pointnum = []
    instance_cls = []
    instance_box = []
    instance_sphere = []
    ...
    centered_coords = coords_float# - center[None,:]( centered around 0,0,0)
    #normalizer_ = centered_coords.max()
    rays_gt_list = []
    ...
    for i_ in range(instance_num):
        ...
        '''
        sph_label, pointwise_angular_gt, ious_upper_bound = get_sph_gt( coords_float_i_centered, centroid_centered, rays_width, rays_height, 
                                                     angles_ref, chk_iou=True, all_points=coords_float, inst_loc=inst_idx_i) # Getting the cartesian coords(centered) of sphere for each predefined labels
        
        if ious_upper_bound[1] < 0.99:
            pdb.set_trace()
        '''
        sph_label, pointwise_angular_gt, ious_upper_bound, rays_gt = get_sph_gt( coords_float_i_centered, centroid_centered, rays_width, rays_height, 
                                                     angles_ref, chk_iou=False, all_points=coords_float, inst_loc=inst_idx_i) # Getting the cartesian coords(centered) of sphere for each predefined labels
        ...
        sph_shift = sph_label[None,:] - coords_float_i_centered[:,None,:].repeat(1,rays_width * rays_height, 1)# using each centered coord as a center, getting distance to sphere for each predefined angle.
        
        sph_shift_center = centroid[None,:] - coords_float_i_centered
        spherical_labels[inst_idx_i, :-1] = sph_shift# /normalizer_
        spherical_labels[inst_idx_i, -1] = sph_shift_center

        ###saniy check
        #sanity_center = sph_shift_center + coords_float_i_centered
        #sanity_shift = sph_shift + coords_float_i_centered[:,None,:].repeat(1,rays_width * rays_height, 1)
        #sanity_recovered = sanity_center[:,None,:] + sanity_shift
        #pdb.set_trace()
        ###
    ...
    if is_analyze:
        return instance_cls, instance_box, instance_sphere, centroid_offset_labels, angular_labels, spherical_labels, rays_gt_list[:,:,0]
    return instance_cls, instance_box, instance_sphere, centroid_offset_labels, angular_labels, corners_offset_labels, spherical_labels, rays_gt_list#corners_offset_labels

def get_instance_info_polarnet_exp2(coords_float, instance_labels, semantic_labels, rays_width, rays_height, angles_ref, label_shift=2, is_analyze=False):
    instance_pointnum = []
    instance_cls = []
    instance_box = []
    instance_sphere = []
    ...
    for i_ in range(instance_num):
        ...
        inst_label = semantic_labels[cls_idx].clone()
        if inst_label != -100:
            inst_label = inst_label - label_shift
        semantic_labels[inst_idx_i] = inst_label
        instance_cls.append(semantic_labels[cls_idx])
        rays_gt_list.append(rays_gt)
    instance_cls = torch.tensor(instance_cls, device=coords_float.device)
    instance_box = torch.stack(instance_box, dim=0)  # N, 6
...
def get_instance_info_polarnet_exp3(coords_float, instance_labels, semantic_labels, rays_width, rays_height, angles_ref, label_shift=2, is_analyze=False):
    instance_pointnum = []
    instance_cls = []
    instance_box = []
    instance_sphere = []
    ...
    for i_ in range(instance_num):
        ...
        #if (rays_gt>1e-6).sum() == 0:
        #    continue
        #rays_gt = smooth_label(rays_gt)
        #pdb.set_trace()
        min_xyz_i = coords_float_i.min(dim=0)[0]
        max_xyz_i = coords_float_i.max(dim=0)[0]
        ...
        sph_shift_center = centroid[None,:] - coords_float_i_centered
        spherical_labels[inst_idx_i, :-1,:] = rays_gt
        spherical_labels[inst_idx_i, -1] = sph_shift_center
        spherical_mask[inst_idx_i] = centers_mask
        ###saniy check
        #sanity_center = sph_shift_center + coords_float_i_centered
        ...
        rays_gt_list.append(rays_gt)
    #semantic_labels[semantic_labels== -100] = 18
    instance_cls = torch.tensor(instance_cls, device=coords_float.device)
    instance_box = torch.stack(instance_box, dim=0)  # N, 6
    ...
    #chk_center = spherical_labels[:,-1] + centered_coords
    #chk_center = chk_center[spherical_mask == 1]
    #dist = torch.cdist(chk_center, instance_sphere[:,-1])
    #dist, _ = torch.min(dist,1)
   

    if is_analyze:
        return instance_cls, instance_box, instance_sphere, centroid_offset_labels, angular_labels, spherical_labels, rays_gt_list[:,:,0], semantic_labels
    return instance_cls, instance_box, instance_sphere, centroid_offset_labels, angular_labels, corners_offset_labels, spherical_labels, rays_gt_list, spherical_mask, semantic_labels#corners_offset_labels

def get_instance_info_polarnet_exp5(coords_float, instance_labels, semantic_labels, rays_width, rays_height, angles_ref, label_shift=2, is_analyze=False):
    instance_pointnum = []
    instance_cls = []
    instance_box = []
    instance_sphere = []
...
def get_instance_info_dyco_ray(coords_float, batch_idxs, instance_labels, semantic_labels, rays_width, rays_height, angles_ref, label_shift=2, is_analyze=False, is_vis=False):
    ...
    for idx in range(batch_size):
        ...
        inst_ids_b = instance_labels[batch_idxs==idx]
        inst_ids_b = inst_ids_b[inst_ids_b != -100]
        max_inst_id = inst_ids_b.max().item()
        min_inst_id = inst_ids_b.min().item()
        instance_idxs.append([min_inst_id, max_inst_id])
    centers = []
    '''
    for i_ in range(instance_num):
        ...
        if is_vis:
            ...
            #get_sph_gt is taking a lot of time
            sph_label, pointwise_angular_gt, ious_upper_bound, rays_gt = get_sph_gt( coords_float_i_centered, centroid_centered, rays_width, rays_height, 
    ...
    for idx in range(batch_size):
        ...
        centers_b = centers[instance_idxs[idx][0]: instance_idxs[idx][1]]
        max_obj_idx = instance_idxs[idx][1]
        min_obj_idx = instance_idxs[idx][0]
        instance_labels_mask = (instance_labels>=min_obj_idx) * (instance_labels<=max_obj_idx)
        points = coords_float[instance_labels_mask]
    ...
    '''
    #print('preprocessing time : {0} time1: {1} time2: {2}'.format(time.time() - start_time, time1 - start_processing_time, time2 - time1))
    #semantic_labels[semantic_labels== -100] = 18
    instance_cls = torch.tensor(instance_cls, device=coords_float.device)
    instance_box = torch.stack(instance_box, dim=0)  # N, 6
...
def get_instance_info_dyco_ray_mask(coords_float, instance_labels, semantic_labels, rays_width, rays_height, angles_ref, num_range, label_shift=2, is_analyze=False):
    instance_pointnum = []
    instance_cls = []
    instance_box = []
    instance_sphere = []
    ...
    for i_ in range(instance_num):
        ...
        if ious_upper_bound[1] < 0.99:
            pdb.set_trace()
        '''
        sph_label, pointwise_angular_gt, ious_upper_bound, rays_gt, angles_num_each, ray_mask, ray_mask_indice = get_sph_gt_ray( coords_float_i_centered, centroid_centered, rays_width, rays_height, 
                                                     angles_ref, num_range, chk_iou=False, all_points=coords_float, inst_loc=inst_idx_i) # Getting the cartesian coords(centered) of sphere for each predefined labels
        

        ##############
        
        centered_points = coords_float - centroid[None,:]
        new_pts = cartesian_to_spherical(centered_points)
        old_pts = spherical_to_cartesian(new_pts[:,3:])
        ...
        new_pts[:,5] += 180
        dists = new_pts[:,3]
        dist_label_anchors = rays_gt[:,0]
        
        _, angles_num_each = divide_with_angle_vectorize(new_pts[:,3:], rays_width, rays_height, return_ref=True, cartesian_ref=coords_float)
    
        mask_indice_from_ray = get_mask_indice_inside_ray(dists, dist_label_anchors, ray_mask, angles_num_each, rays_width*rays_height, num_range)
        

        box_in_3D_gt = get_3D_locs_from_rays(dist_label_anchors[None,:], angles_ref, centroid[None,:])
        ray_box_gt = torch.cat([box_in_3D_gt, centroid[None, None,:]],1)
        ray_box_gt = ray_box_gt.reshape(len(ray_box_gt),-1)
        masks_ray_ = get_mask_from_polar_single(coords_float, ray_box_gt, rays_width, rays_height, 
                                                                   is_angular=False, pointwise_angular=torch.ones(len(coords_float),2).float().cuda(),
                                                                    sim_ths = 0.8 )
        
        mask_gt = torch.zeros(len(coords_float)).float().cuda()
        mask_ray_indice = torch.zeros(len(coords_float)).float().cuda()

        mask_gt[inst_idx_i] = 1
        mask_ray_indice[mask_indice_from_ray] = 1

        iou_ray = get_iou(mask_gt[None,:],masks_ray_)
        iou_ray_inside = get_iou(mask_gt[None,:], mask_ray_indice[None,:])
        print('{0}, {1}'.format(iou_ray, iou_ray_inside))
        pdb.set_trace()
        #############
        
    
    
        rays_mask_list.append(ray_mask)
        #if (rays_gt>1e-6).sum() == 0:
        #    continue
        #rays_gt = smooth_label(rays_gt)
        rays_gt = torch.cat([rays_gt, centroid[:,None]],0)
    ...
    rays_mask_list = torch.stack(rays_mask_list,0)
    #chk_center = spherical_labels[:,-1] + centered_coords
    #chk_center = chk_center[spherical_mask == 1]
    #dist = torch.cdist(chk_center, instance_sphere[:,-1])
    #dist, _ = torch.min(dist,1)
    ...
    if is_analyze:
        return instance_cls, instance_box, instance_sphere, centroid_offset_labels, angular_labels, spherical_labels, rays_gt_list[:,:,0], semantic_labels
    return instance_cls, instance_box, instance_sphere, centroid_offset_labels, angular_labels, corners_offset_labels, spherical_labels, rays_gt_list, spherical_mask, semantic_labels, rays_mask_list#corners_offset_labels


def get_instance_info_polarnet(coords_float, instance_labels, semantic_labels, rays_width, rays_height, angles_ref, label_shift=2, is_analyze=False):
    instance_pointnum = []
    instance_cls = []
    instance_box = []
    instance_sphere = []
    ...
    if is_analyze:
        return instance_cls, instance_box, instance_sphere, centroid_offset_labels, angular_labels, spherical_labels, rays_gt_list[:,:,0]
    return instance_cls, instance_box, instance_sphere, centroid_offset_labels, angular_labels, spherical_labels#corners_offset_labels
...
def get_spp_gt_ray_final(
    ...
    for b in range(batch_size):
        ...
        unique_inst = torch.unique(instance_labels_b)
        unique_inst = unique_inst[unique_inst != ignore_label]
        #if unique_inst.max().item() > len(instance_cls):
        #    pdb.set_trace()
        #print('{0} , {1}'.format(unique_inst.max(), len(instance_cls)))
        #if instance_cls[unique_inst].max() > len(unique_inst):
        #   pdb.set_trace()
        try:
            unique_inst = unique_inst[instance_cls[unique_inst] >= 0]
        except:
...
def get_spp_gt_rays(
    instance_labels, spps, instance_cls, instance_box, rays_labels, angular_labels, coords, batch_offsets, batch_size, spp_voxels, ignore_label=-100, pool=True
):
    # original_type = instance_box.dtype
    spp_inst_mask_arr = []
    unique_voxels_gt = []
    ...
    for b in range(batch_size):
        ...
        instance_labels_b = instance_labels[start:end]
        angular_labels_b = angular_labels[start:end]
        ...
        unique_inst = unique_inst[unique_inst != ignore_label]
        #pdb.set_trace()
        #spp_voxels = coords_b[spp_b_unique] ### This seems wrong
        ...
        instance_cls_b = instance_cls[unique_inst]
        instance_box_b = instance_box[unique_inst]
        rays_labels_b = rays_labels[unique_inst]

        spp_mask_labels_b = torch.zeros((n_inst_gt, n_spp), device=instance_labels.device, dtype=torch.float)
        spp_angular_labels_b = torch.zeros((n_spp,2), device=instance_labels.device, dtype=torch.float)
        spp_angular_labels_b[:,:] = -100
        #spp_angular_labels_b[:,:] = 1e-6
        overlap_cnt = torch.zeros(len(spp_voxels))
        ...
        for i, uni_id in enumerate(unique_inst):
            ...
            unique_voxel_inst = torch.unique(spp_b[mask_])
            
            center = rays_labels_b[i,-3:,0]
            

            cond_ = spp_mask_ >= 0.5 # Finding super-points that have more than half of its belonging voxels True as gt mask 
            locs_ = torch.where(cond_)[0]
            unique_voxels_gt.append(locs_)

            overlap_cnt[locs_] += 1
            #unique_voxels_i = coords_b[locs_]### It shouldn't be here because, coords_b has entire points whereas locs_ is from subsample
            
            unique_voxels_i = spp_voxels[locs_]
            
            #centroid = torch.mean(unique_voxels_i)
            #sph_label, pointwise_angular_gt, ious_upper_bound, rays_gt = get_sph_gt( coords_float_i_centered, centroid_centered, rays_width, rays_height, 
            #                                         angles_ref, chk_iou=False, all_points=coords_float, inst_loc=inst_idx_i)

            angular_gt_inst = get_angular_gt(unique_voxels_i, center)
            
            #if i == 0:
            #    print('center:{0} gt_ist:{1} voxels: {2}'.format(center,  locs_, unique_voxels_i[0]))
            
            spp_angular_labels_b[locs_] = angular_gt_inst
        ...
        spp_inst_mask_arr.append(
            {
                ...
                "box": instance_box_b,
                'rays': rays_labels_b,
                'angular_gt': spp_angular_labels_b
            }
        )

    return spp_inst_mask_arr
def get_spp_gt_rays2(
    instance_labels, spps, instance_cls, instance_box, rays_labels, angular_labels, coords, batch_offsets, batch_size, spp_voxels, ignore_label=-100, pool=True
):
    # original_type = instance_box.dtype
    spp_inst_mask_arr = []
    ...
    for b in range(batch_size):
        ...
        #angular_x = custom_scatter_mean(angular_pred_b[:,0], spp_b, pool=pool, output_type=torch.float32)
        #angular_y = custom_scatter_mean(angular_pred_b[:,1], spp_b, pool=pool, output_type=torch.float32)
        #angular_xy_pred = torch.cat([angular_x[:,None], angular_y[:,None]],1)
        #angular_pred_list.append(angular_xy_pred)
    ...
    return spp_inst_mask_arr, unique_voxels_gt
...
def get_spp_gt_rays_cluster(
    instance_labels, spps, instance_cls, instance_box, rays_labels, angular_labels, coords, batch_offsets, batch_size, spp_voxels, spp_batch_offsets, angles_ref, rays_width, rays_height, ignore_label=-100, pool=True
):
    # original_type = instance_box.dtype
    spp_inst_mask_arr = []
    unique_voxels_gt = []
    ...
    for b in range(batch_size):
        start, end = batch_offsets[b], batch_offsets[b + 1]
        start_spp, end_spp = spp_batch_offsets[b], spp_batch_offsets[b+1]

        spp_voxels_b = spp_voxels[start_spp : end_spp]
        ...
        #angular_xy_pred = torch.cat([angular_x[:,None], angular_y[:,None]],1)
        #angular_pred_list.append(angular_xy_pred)
        target_mask_cluster = torch.zeros((n_inst_gt, n_spp,2), device=instance_labels.device, dtype=torch.float)
        target_mask_cluster[:,:,0] = 0
        ...
        for i, uni_id in enumerate(unique_inst):
            ...
            #cond_ = spp_mask_ >= 0.3 # Finding super-points that have more than half of its belonging voxels True as gt mask 
            cond_ = spp_mask_ >= 0.5
            ...
            target_mask_cluster[i,cond_,0] = 1
            
            
       
        #rays_gt_list = torch.cat(rays_gt_list)
        ...
        spp_inst_mask_arr.append(
            {
                ...
                #'rays': rays_gt_list,
                'rays': rays_labels_b,
                'angular_gt': target_mask_cluster
            }
        )
    
    #all_rays_gts = torch.cat(all_rays_gts)
    
    return spp_inst_mask_arr, unique_voxels_gt, []#all_rays_gts 


def get_spp_gt_rays_cluster_inside(
    instance_labels, spps, instance_cls, instance_box, rays_labels, angular_labels, ray_mask_label, coords, batch_offsets, batch_size, spp_voxels, spp_batch_offsets, angles_ref, rays_width, rays_height, ignore_label=-100, pool=True
):
    # original_type = instance_box.dtype
    spp_inst_mask_arr = []
    unique_voxels_gt = []
    ...
    for b in range(batch_size):
        ...
        instance_box_b = instance_box[unique_inst]
        rays_labels_b = rays_labels[unique_inst]
        ray_mask_label_b = ray_mask_label[unique_inst]
        ...
        box_in_3D = get_3D_locs_from_rays(rays_labels_b[:,:-3,0], angles_ref, rays_labels_b[:,-3:,0])
               
        ray_box_preds = torch.cat([box_in_3D, rays_labels_b[:,None,-3:,0]],1)
        ray_box_preds = ray_box_preds.reshape(len(ray_box_preds),-1)
        

        masks_ray_ = get_mask_from_polar_single(coords_b, ray_box_preds, rays_width, rays_height, 
                                                                is_angular=False, pointwise_angular=torch.ones(len(coords_b),2).float().cuda(),
                                                                sim_ths = 0.8 )
        '''
        iou_upper_list = []
        for i, uni_id in enumerate(unique_inst):
            ...
            points_ = coords_b[mask_]
            center = torch.mean(points_,0)

            spp_mask_ = custom_scatter_mean(mask_, spp_b, pool=pool, output_type=torch.float32)
            
            centered_points = spp_voxels_b - center[None,:]
            sph_coords_ray_dist = cartesian_to_spherical(centered_points)[:,3]
            ...
            #mask_ray = masks_ray_[i]
            #iou_upper, initial_mask = get_iou_with_conf(mask_, spp_mask_, spp_b, conf_val=0.5)
            #iou_upper_with_ray, refined_mask = get_iou_with_conf(mask_, spp_mask_, spp_b, conf_val=0.5, is_ray=True, coords=coords_b, ray_mask=mask_ray, ray_center=rays_labels_b[i])
            #spp_mask_labels_b[i] = refined_mask.float()
            
           
            
            #target_mask_cluster[i,:,1:] = center
            target_mask_cluster[i,:,1] = sph_coords_ray_dist
        #pdb.set_trace()   
        #pdb.set_trace()
        spp_inst_mask_arr.append(
            {
                ...
                'rays': rays_labels_b,
                'angular_gt': target_mask_cluster,
                'ray_inside_mask': ray_mask_label_b
            }
        )
   
    return spp_inst_mask_arr, unique_voxels_gt
def get_spp_gt_rays_cls(
    instance_labels, spps, instance_cls, instance_box, rays_labels, rays_mask_labels, batch_offsets, batch_size, ignore_label=-100, pool=True
):
    # original_type = instance_box.dtype
    spp_inst_mask_arr = []
    for b in range(batch_size):
        ...
        instance_box_b = instance_box[unique_inst]
        rays_labels_b = rays_labels[unique_inst]
        rays_mask_labels_b = rays_mask_labels[unique_inst]
