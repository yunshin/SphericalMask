...
def load_pred(base_path, scan_id, benchmark_indice):
    ...
    if os.path.exists(os.path.join(base_path,scan_id+'.txt')) :
        ...
        for idx in range(len(lines)):
            ...
            new_mask_path = pred_mask_path.replace('predicted_masks','new_predicted_masks')
            new_mask_path = os.path.join(base_path,pred_mask_path)
...
def main():
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            #print('{0}: {1}'.format(new_scans, scan_id))
            #if scan_id == 'scene0652_00':
            #    pdb.set_trace()
            ...
            ############Loading part
            pred_insts_load = load_pred(load_path, scan_id, dataset.BENCHMARK_SEMANTIC_IDXS)
