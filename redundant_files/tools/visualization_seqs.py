...
def get_coords_color(opt):
    if opt.dataset == "s3dis":
        ...
        if opt.dataset == 'scannetv2':
            input_file = os.path.join("/root/src/SoftGroup/dataset", opt.dataset, opt.data_split, opt.room_name + "_inst_nostuff.pth")
        else:
    ...
    # same color order according to instance pointnum
        ...
        instance_file = os.path.join(opt.prediction_path,  opt.room_name + ".txt")
        ...
        for i_ in range(len(masks) - 1, -1, -1):
            ...
            mask_path = os.path.join(opt.prediction_path,  masks[i][0])
    ...
    if False and opt.data_split != "test":
        pdb.set_trace()
        sem_valid = label != -100
        xyz = xyz[sem_valid]
        rgb = rgb[sem_valid]

    return xyz, rgb, boxes, sort_idx, ori_rgb


def write_ply(verts, colors, indices, pred_sort_indice, ori_rgb, output_file):
    if colors is None:
        colors = np.zeros_like(verts)
    if indices is None:
        indices = []
    np.save(output_file.replace('.ply','_rgb.npy'),ori_rgb)
    np.save(output_file.replace('.ply','_pred_sort_indice.npy'),pred_sort_indice)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_base", default="", help="output point cloud file in FILE.ply format")
    opt = parser.parse_args()


    #room_names_cand = os.listdir(os.path.join(opt.prediction_path, "pred_instance"))
    room_names_cand = os.listdir(os.path.join(opt.prediction_path))
    room_names = []
    for idx in range(len(room_names_cand)):
        if '.txt' in room_names_cand[idx]:
            room_names.append(room_names_cand[idx])
    

    for idx in range(len(room_names)):

        
        #if idx> 5:
        #    break
        opt.room_name = room_names[idx].split('.')[0]
        opt.out = os.path.join(opt.out_base, opt.room_name) + '.ply'
        xyz, rgb, boxes, pred_sort_indice, ori_rgb = get_coords_color(opt)
        ...
        if opt.out != "":
            ...
            write_ply(points, colors, None, pred_sort_indice, ori_rgb, opt.out)
        else:
