...
from isbnet.model.model_utils import matrix_nms_wo_box
...
def save_single_instance(root, scan_id, insts, benchmark_sem_id):
    ...
    for i, inst in enumerate(insts):
        #print('scan_id:{0} inst scan_id: {1}'.format(scan_id, inst["scan_id"]))
...
def main():
    ...
    if False and load_path is not None:
        new_scans = []
    ...
    for idx_room in range(data_length):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(dataloader):
                ...
                pred_insts_load, is_load, all_masks, all_cls, all_scores = load_pred(load_path, scan_id, dataset.BENCHMARK_SEMANTIC_IDXS)
                
                proposals, cls_, scores_, survived_indice = matrix_nms_wo_box(all_masks.cuda(), all_cls.cuda(), all_scores.cuda())
                proposals = proposals.cpu()
                nms_instances = []
                for idx_nms in range(len(proposals)):
                    pred = {}
                    pred['scan_id'] = scan_id
                    
                    pred['label_id'] = int(cls_[idx_nms].item())#label_num_benchmark
                    pred['conf'] = scores_[idx_nms].item()
                    print('{0}: {1}'.format(idx_nms,proposals[idx_nms].sum()))
                    try:
                        pred['pred_mask'] = rle_encode_gpu(proposals[idx_nms].float().cuda())
                    except:
                        pdb.set_trace()
                    nms_instances.append(pred)
 
                if is_load == True:
                    
                    #res["pred_instances"] = pred_insts_load
                    res["pred_instances"] = nms_instances
                else:
                    pdb.set_trace()
                ############
