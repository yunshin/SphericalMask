...
def train(epoch, model, optimizer, scheduler, scaler, train_loader, cfg, logger, writer, best_scores):
    ...
    for i, batch in enumerate(train_loader, start=1):
        ...
        min_grad = 100
        max_name, min_name = None, None
        for name, params in model.named_parameters():
         
            param_norm = params.grad.detach().data.norm()
            if torch.isnan(param_norm).any():
                pdb.set_trace()
                print('NaN : {0} max : {1}'.format(name, torch.abs(param_norm).max().item()))

            if torch.abs(param_norm).max() > max_grad:
                max_grad = torch.abs(param_norm).max().item()
                max_name = name
            if torch.abs(param_norm).min() < min_grad: 
                min_grad = torch.abs(param_norm).min().item()
                min_name = name
        #print('max name : {0} max grad: {1}    min name: {2} min grad: {3}'.format(
        #    max_name, max_grad, min_name, min_grad))
        #if max_grad < 1:
        #    pdb.set_trace()
        optimizer.step()
        '''
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            if vis_save:
                ...
                save_path = '/root/data/pcdet_data/scannet_pcdet_data/qual_vis_ours_train/'
                #save_path = '/root/data/pcdet_data/scannet_pcdet_data/qual_isbnet_train/'
                np.save('{0}/{1}_rgb.npy'.format(save_path, batch['scan_ids'][0]), res['feats_np'])
                np.save('{0}/{1}_cls.npy'.format(save_path, batch['scan_ids'][0]), res['instance_cls_np'])
                if 'rays_labels_np' in res:
                    np.save('{0}/{1}_rays.npy'.format(save_path, batch['scan_ids'][0]), res['rays_labels_np'])
                np.save('{0}/{1}_sem_label.npy'.format(save_path, batch['scan_ids'][0]), res['semantic_labels_np'])
                np.save('{0}/{1}_inst_label.npy'.format(save_path, batch['scan_ids'][0]), res['instance_labels_np'])
                np.save('{0}/{1}_points.npy'.format(save_path, batch['scan_ids'][0]), res['coords_float_np'])
                if 'instance_sphere_np' in res:
                    np.save('{0}/{1}_sphere.npy'.format(save_path, batch['scan_ids'][0]), res['instance_sphere_np'])
                if 'real_points' in res:
                    np.save('{0}/{1}_real_points.npy'.format(save_path, batch['scan_ids'][0]), np.array(res['real_points']))
                with open('{0}/{1}_pred.pkl'.format(save_path, batch['scan_ids'][0]), 'wb') as handle:
                    pickle.dump(res['pred_instances'], handle, protocol=pickle.HIGHEST_PROTOCOL)
                    
                print('{0}/{1}_inst_label.npy'.format(save_path, batch['scan_ids'][0]))
                #pdb.set_trace()
            if i % 10 == 0:
...
def main():
    ...
    cfg.data.test['debug'] = True
    val_set2 = build_dataset(cfg.data.test, logger)
    ...
    train_val_loader = build_dataloader(val_set, training=False, dist=False, **cfg.dataloader.test)
    train_val_loader_vis = build_dataloader(train_set, training=False, dist=False, **cfg.dataloader.train)
    train_val_loader_debug = build_dataloader(val_set2, training=True, dist=False, **cfg.dataloader.train)
    #pdb.set_trace()
    # optim
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if  epoch%1 == 0 :
            ...
            #if 'paper' in cfg and cfg.paper:
            #    print('Stopped for comparing the result')
            #    pdb.set_trace()
            print(best_epoch)
            #if ap_3 > best_scores['ray+inclusion_mask']:
            #    best_scores['ray+inclusion_mask'] = ap_3
            #validate(epoch, model, optimizer, train_val_loader, cfg, logger, writer)
            cnt_eval += 1
        #if cnt_eval == 5:
        #    pdb.set_trace()
        writer.flush()
