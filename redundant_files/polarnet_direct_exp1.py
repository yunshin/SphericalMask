...
from .model_utils import (
    ...
    get_batch_offsets,
    get_instance_info,
    get_instance_info_polarnet_exp1,
    get_spp_gt,
    get_subsample_gt,
    nms,
    random_downsample,
...
import pdb
import time
class PolarNet_Direct_EXP1(nn.Module):
    def __init__(
        self,
        is_mask=True,
        model_name='polarnet_direct_exp1',
        channels=32,
        num_blocks=7,
        semantic_only=False,
        semantic_classes=20,
        ...
        with open('/root/data/pcdet_data/scannet_pcdet_data/rays_anchors.pkl', 'rb') as f:
    ...
    def forward_train(
        ...
        **kwargs
        ...
        instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, corners_offset_labels_sph, rays_labels = get_instance_info_dyco_ray(
    ...
    def forward_test(
        ...
        **kwargs
        ...
        if self.is_mask == False:
            ...
            instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, corners_offset_labels_sph, corners_offset_labels = get_instance_info_dyco_ray(
                voxel_coords_float, voxel_instance_labels, voxel_semantic_labels, self.rays_width, self.rays_height, self.angles_ref, label_shift=self.label_shift
            )
        ...
        for i in range(len(n_sample_arr)):
            ...
            pred_category = torch.argmax(cls_logits,1)
            max_gt2pred, iou_gt2pred, closest_label = manual_iou(masks_ray_, instance_labels, v2p_map, p2v_map[:,1], pred_category, instance_cls, p2v=True)
            closest_label = torch.cat([torch.arange(len(closest_label)).long().cuda()[:,None], closest_label[:,None]],1 )
          
            #cls_logits[:,:] = 0
            #cls_logits[closest_label[:,0], closest_label[:,1]] = 3
         
            '''
            mask_best = self.best_iou < max_gt2pred
            
            for idx_best in range(len(mask_best)):
                if mask_best[idx_best]:
                    print('{0} updated : {1} -> {2}'.format(idx_best, self.best_iou[idx_best], max_gt2pred[idx_best]))
                    self.best_iou[idx_best] = max_gt2pred[idx_best]
            '''
            #print(max_gt2pred)
    ...
    def get_instance(
        ...
        #print('scores :{0}  cls_pred : {1}, scores val : {2}'.format(scores_cond.shape, cls_pred.shape, scores_cond))
