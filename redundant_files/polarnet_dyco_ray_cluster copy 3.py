...
import pdb
import numpy as np
class PolarNet_Dyco_Ray_Cluster(nn.Module):
    def __init__(
        self,
        ...
        self.init_weights()
        #self.criterion.set_ray_head(self.inst_inside_mask)
        ...
        if os.path.exists('/root/data/pcdet_data/scannet_pcdet_data/rays_anchors_max_vals_{0}_{1}.npy'.format(rays_height, rays_width)):
            self.rays_anchors_tensor = np.load('/root/data/pcdet_data/scannet_pcdet_data/rays_anchors_max_vals_{0}_{1}.npy'.format(rays_height, rays_width))
            self.rays_anchors_tensor = torch.tensor(self.rays_anchors_tensor).float().cuda()
            self.criterion.set_anchor(self.rays_anchors_tensor)
        else:    
            ...
            for key in self.anchors_max:
                
                self.rays_anchors_tensor[key] = torch.tensor(self.anchors_max[key][:-3]).float().cuda()
            self.criterion.set_anchor(self.rays_anchors_tensor)
            
            #print('skip anchor loading at 247 polarnet')
        if False and os.path.exists('/root/data/pcdet_data/scannet_pcdet_data/rays_anchors_{0}_{1}.pkl'.format(rays_height, rays_width)):
            ...
            self.criterion.set_anchor(self.rays_anchors_tensor, self.rays_norm_params, self.anchors_cdf_tensor)
    ...
    def init_dyco_angular(self):
        ...
        weight_nums_angular = [(self.mask_dim_out_angular + 3 + 3  ) * self.mask_dim_out_angular, self.mask_dim_out_angular * self.angular_feature_dim]
        bias_nums_angular = [self.mask_dim_out_angular, self.angular_feature_dim]
        ...
        self.inst_inside_head = GenericMLP(
                input_dim=256,
                hidden_dims=[256, 256],
                norm_fn_name="bn1d",
                activation="relu",
    ...
    def forward_train(
        ...
        **kwargs
        ...
        if self.use_spp_pool:
            ...
            dc_inst_mask_arr, _, rays_labels_ = get_spp_gt_rays_cluster(
        ...
        #dc_mask_features_angular = self.mask_tower_angular(torch.unsqueeze(dc_output_feats, dim=2).permute(2, 1, 0)).permute(2, 1, 0)
        
        dc_mask_features_permute = self.inst_inside_mask(F.relu(dc_mask_features.detach().squeeze()))
        
        #cls_logits, mask_logits, mask_ray_logits, conf_logits, box_preds, rays_pred, proto_coff, dc_feats = self.forward_head(
        #    query_feats, query_locs, dc_mask_features, dc_mask_features_ray, dc_mask_features_angular, dc_coords_float, dc_box_preds, dc_batch_offsets
        #)
        cls_logits, mask_logits, mask_ray_logits, conf_logits, box_preds, rays_pred, proto_coff, dc_feats = self.forward_head(
        ...
        model_outputs.update(
            dict(
                ...
                #dc_mask_features_angular=dc_output_feats
                #dc_mask_features_angular=dc_mask_features_angular
                #dc_mask_features_angular=dc_mask_features
    ...
    def forward_test(
        ...
        **kwargs
        ...
        #self.rays_anchors_tensor = self.criterion.get_anchors()
        voxel_semantic_labels = semantic_labels[p2v_map[:, 1].long()]
        voxel_instance_labels = instance_labels[p2v_map[:, 1].long()]
        voxel_instance_labels = get_cropped_instance_label(voxel_instance_labels)
        ...
        dc_inst_mask_arr, _, _ = get_spp_gt_rays_cluster(
                voxel_instance_labels,
                voxel_spps,
                instance_cls,
                instance_box,
        ...
        rays_labels = rays_labels[instance_cls != -100]
        cls_labels = instance_cls[instance_cls != -100]
        for i in range(len(n_sample_arr)):
            ...
            cls_logits, mask_logits, mask_ray_logits, conf_logits, box_preds, rays_pred, proto_coff, dc_feats = forward_head_outputs
            
            #if type == 1:
            #    mask_logits += angular_pred
            mask_ray_logits = rays_pred
            
            #rays_pred = mask_ray_logits
            ...
            #####
            ray_box_preds = rays_pred
            #########
            #ray_box_preds: torch.Size([256, 75])

            _, cls_pred = torch.max(cls_logits,1)
            
            #ray_box_preds = ray_box_preds[torch.arange(len(ray_box_preds)).long().cuda(),cls_pred]
            ...
            #closest_gt2pred = torch.cdist( query_locs[0], ray_box_preds[:,-3:])
            #closest_gt2pred = torch.argmin(closest_gt2pred, 1)
            
            ray_corres_gt = rays_labels[closest_gt,:,0]
            cls_labels_corres = cls_labels[closest_gt]

            ###############
            
            cls_logits = torch.zeros_like(cls_logits)
            cls_logits[:] = -4
            
            cls_logits[torch.arange(len(cls_labels_corres)).long().cuda(),cls_labels_corres] = 5
            conf_logits[:] = 0.8
            ...
            masks_proto[:,:] = -100
            if (mask_logits>=0).sum().item() == 0:
                rays_from_mask = try_vectorize_mask2ray(dc_coords_float,  (mask_logits<0)*1.0,  self.rays_width, self.rays_height)
            else:
                rays_from_mask = try_vectorize_mask2ray(dc_coords_float,  (mask_logits>=0)*1.0,  self.rays_width, self.rays_height)
           
            #dist_anchors, angles_num, roi_locs = try_vectorize(dc_coords_float, (mask_logits>=0)*1.0, ray_box_preds, self.rays_width, self.rays_height)            
            #dist_anchors, angles_num, roi_locs = try_vectorize(dc_coords_float, (mask_logits>=0)*1.0, rays_from_mask, self.rays_width, self.rays_height)
            dist_anchors, angles_num, roi_locs = try_vectorize(dc_coords_float, (mask_logits>=0)*1.0, ray_corres_gt.clone(), self.rays_width, self.rays_height)      
            ...
            #roi_pred_total = torch.matmul(F.relu(dc_mask_features_permute[roi_locs[1]].squeeze()), proto_coff.T)#ori
            '''
            feat = dc_feats[roi_locs[0],roi_locs[1]].squeeze()
            feat2 = dc_mask_features_permute[roi_locs[1]].squeeze()
            feats = feat+feat2
            '''
            feats = dc_mask_features_permute[roi_locs[1]].squeeze()
            #roi_pred_total = torch.matmul(F.relu(dc_feats[roi_locs[0],roi_locs[1]].squeeze()), proto_coff.T)
            ...
            for mask_idx in range(len(all_masks_ray_)):
                ...
                #roi_mask_pred = torch.matmul(dc_mask_features_angular[roi_indice_idx].squeeze(),proto_coff[mask_idx][:,None])
            ...
            if type == 0: 
                #angular_pred = angular_pred[:,voxel_spps]
                '''
                ...
                max_inst = instance_labels.max().item()
                masks_ray_ = masks_ray_[:, voxel_spps]
                
                max_ious = []
                for idx_inst in range(max_inst+1):
                    mask_t = voxel_instance_labels == idx_inst
                    if instance_cls[idx_inst] == -100:
                        continue
                    
                    if (conf_logits > 0).sum().item() == 0:
                        max_ious.append(0)
                        continue
                    ious = get_iou(masks_ray_[conf_logits > 0], mask_t[None,:])
                    max_ious.append(ious.max().item())
                print(max_ious)
              
                mask_logit = masks_ray_[:, voxel_spps]
                
                mask_logits_final.append(mask_logit)
            elif type == 1:
                #mask_logit = masks_proto[:, voxel_spps]
                masks_ray_ = masks_proto[:, voxel_spps]
                '''
                masks_ray_ = get_mask_from_polar_single2(dc_coords_float[None,:,:].repeat(len(angular_pred),1,1), ray_box_preds, mask_logits, self.rays_width, self.rays_height, 
                ...
                masks_ray_ori = get_mask_from_polar_single(dc_coords_float, ray_box_gt, self.rays_width, self.rays_height, 
                                                                   is_angular=False, pointwise_angular=torch.ones(len(dc_coords_float),2).float().cuda(),
                                                                    sim_ths = 0.8 )
                ...
                mask_logit = mask_logits[:, voxel_spps]############why is this on?
                mask_logits_final.append(mask_logit)
            elif type == 2:
    ...
    def forward_head(
        self, queries_features, queries_locs, mask_features_, mask_ray_features_, mask_angular_features_, locs_float_, box_preds_, batch_offsets_, inference=False
    ):
        ...
        #rays_center = rays_center.reshape(rays_center.shape[0], rays_center.shape[1], (self.instance_classes+1),3)
        #rays_pred = rays_pred.reshape(rays_pred.shape[0], rays_pred.shape[1], (self.instance_classes+1),self.rays_height*self.rays_width)
        #rays_center = rays_center + queries_locs[:,:,None,:] ????
        ...
        if inference:
            ...
            '''
            anchor_rays = self.rays_anchors_tensor[cls_pred]
            rays_ = []
            for idx_ray in range(len(cls_pred)):
                #ray = rays_pred[:,idx_ray,cls_pred[idx_ray]]
                ray = rays_pred[:,idx_ray]
                
                ray[0,:-3] = anchor_rays[idx_ray] - (ray[0,:-3])
                rays_.append(ray)
            
            
            rays_pred = torch.cat(rays_)[None,:]
            '''
            rays_pred[:,:,:-3] = torch.clamp(rays_pred[:,:,:-3], 0.01, 10)
        ...
        #proto_coeff = F.relu(controllers_angular)#self.inst_inside_head(F.relu(controllers_angular))
        proto_coeff = controllers_angular
       
        #angular_features = F.relu(self.inst_angular_head(mask_features_))
        mask_logits = []
        mask_ray_logits = []
        angular_preds = []
        mask_feat_list_total = []
        for b in range(batch_size):
            ...
            controller_angular = controllers_angular[b]
            ...
            #weights_angular, biases_angular = self.parse_dynamic_params_angular(controller_angular, self.mask_dim_out_angular)
            ...
            #mask_angular_feature_b = mask_angular_features_[start:end]
            ...
            for i in range(num_chunks):
                ...
                #weights_angular_split = [w[chunk_start:chunk_end] for w in weights_angular]
                #biases_angular_split =  [b[chunk_start:chunk_end] for b in biases_angular]
                
                '''
                angular_logit_ = self.angular_heads_forward(
                    mask_angular_feature_b,
                    weights_angular_split,
                    biases_angular_split,
                    weights_angular_split[0].shape[0],
                    locs_float_b,
                    box_preds_b,
                ...
                '''
                mask_logit_, mask_logit_feat = self.mask_heads_forward(
                    mask_feature_b,
                    weights_split,
                    biases_split,
                ...
                mask_logit_feat = mask_logit_feat.permute(0,2,1)
                mask_feat_list.append(mask_logit_feat)
                '''
                ray_logit_,_ = self.mask_heads_forward(
                    mask_ray_feature_b,
                    weights_ray_split,
                    biases_ray_split,
                    weights_split[0].shape[0],
                ...
                if inference:
                    ...
                    for idx_ray in range(len(cls_pred)):
                        ...
                        mask_inside = angular_logit_[idx_ray,cls_pred[idx_ray]]
                        rays_inside.append(mask_inside[None,:])
                   
                    angular_logit_ = torch.cat(rays_inside)
                    '''
                    if self.is_anchors:
                ...
                #angular_pred.append(angular_logit_)
            mask_feat_list = torch.cat(mask_feat_list,dim=0)
            mask_logit = torch.cat(mask_logit, dim=0)
            #mask_ray_logit = torch.cat(mask_ray_logit, dim=0)
            #angular_pred = torch.cat(angular_pred, dim=0)



            #angular_preds.append(angular_pred)
            mask_logits.append(mask_logit)
            #mask_ray_logits.append(mask_ray_logit)
            mask_feat_list_total.append(mask_feat_list)
        
        if inference:
            return cls_logits[0], mask_logits[0], [], conf_logits[0], queries_box_preds[0], rays_pred[0], proto_coeff[0], mask_feat_list_total[0] #angular_preds[0]
        return cls_logits, mask_logits, mask_ray_logits, conf_logits, queries_box_preds, rays_pred, proto_coeff, mask_feat_list_total#
    ...
    def parse_dynamic_params_angular(self, params, out_channels):
        ...
        weight_splits[0] = weight_splits[0].reshape(num_instances, out_channels + 6, out_channels)
        bias_splits[0] = bias_splits[0].reshape(num_instances, out_channels)
        weight_splits[1] = weight_splits[1].reshape(num_instances, out_channels, self.angular_feature_dim)
        bias_splits[1] = bias_splits[1].reshape(num_instances, self.angular_feature_dim)
