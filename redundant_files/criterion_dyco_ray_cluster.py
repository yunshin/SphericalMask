    ...
    def get_loss_new(self, dc_coords_float_b, inst_label, ray_label, mask_feat_b, dc_proto_features_b, proto_coff_b, mask_logit_pred, ray_pred ):
        ...
        if False and (mask_logit_pred>=0).sum(1).min().item() == 0:
            ...
            with torch.no_grad():
                ...
                ray_pred_chk = ray_pred.clone().detach()
                ray_pred_chk = torch.clamp(ray_pred_chk, 0,100)
                #ray_pred_chk[ray_pred_chk<0] = 0
              
                rays_refined = rays_from_mask_pred[:,:-3] + ray_pred_chk
        ...
        with torch.no_grad():
            ...
            dist_anchors_pred, angles_num_pred, roi_locs_pred, _ = try_vectorize(dc_coords_float_b.clone(), inst_label, rays_from_mask, self.rays_width, self.rays_height)
            #dist_anchors_pred_, angles_num_pred_, roi_locs_pred_debug, _ = try_vectorize(dc_coords_float_b.clone(), inst_label, rays_from_mask_pred, self.rays_width, self.rays_height)
            #dist_anchors_pred_, angles_num_pred_, roi_locs_pred_refined, _ = try_vectorize(dc_coords_float_b.clone(), inst_label, rays_refined, self.rays_width, self.rays_height)
        ...
        #if len(roi_locs_pred_refined[0]) == 0:
        #    print(roi_locs_pred_refined[0])
        feats1 = mask_feat_b[roi_locs_pred[0],roi_locs_pred[1]]
        feats2 = dc_proto_features_b[roi_locs_pred[1]].squeeze()
       
        #feats1 = mask_feat_b[roi_locs_pred_refined[0],roi_locs_pred_refined[1]]
        #feats2 = dc_proto_features_b[roi_locs_pred_refined[1]].squeeze()
        feats = feats1 + feats2
        #feats = feats1
        ...
        #2
        #roi_pred_total = torch.matmul(proto_coff_b, feats.T)
        #roi_pred_total = roi_pred_total[roi_locs_pred[0],torch.arange(roi_pred_total.shape[1]).long().cuda()]
        
        #3
        #roi_pred_total = feats.mean(1)

        #4
        #roi_pred_total = self.mask_head(feats1[:,None,:], feats2[:,None,:], feats2[:,None,:]).squeeze()
        #roi_pred_total = self.mask_head(feats1[:,None,:], feats1[:,None,:], feats2[:,None,:]).squeeze()
        ...
        if only_false == False:
            ...
            pred_mask_second = (roi_pred_total >= 0)*1.0
            FP_mask = ((pred_mask_second == 1) * (roi_gt_total == 0))*1.0
            FN_mask = ((pred_mask_second == 0) * (roi_gt_total == 1))*1.0

            false_mask = (FP_mask + FN_mask) >= 1
            if false_mask.sum().item() == 0:
                print('false 0')
                dice_loss_ray = compute_dice_loss(roi_pred_total[None,:], roi_gt_total[None,:], 1)
                ...
                dice_loss_ray = compute_dice_loss(roi_pred_total[false_mask][None,:], roi_gt_total[false_mask][None,:], 1)
                
                ray_bce_loss = F.binary_cross_entropy_with_logits(roi_pred_total[false_mask], roi_gt_total[false_mask], reduction="none")
        
        pred_mask = roi_pred_total >= 0
        acc = (pred_mask == roi_gt_total).sum() / len(roi_gt_total)
        #print(acc.item())
        ray_bce_loss = ray_bce_loss.sum() / roi_gt_total.sum()
        ray_pred_loss = F.l1_loss(ray_pred, diff)
