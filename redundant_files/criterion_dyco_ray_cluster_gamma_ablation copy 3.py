    ...
    def get_loss_new(self, dc_coords_float_b, inst_label, ray_label, mask_feat_b, dc_proto_features_b, proto_coff_b, mask_logit_pred, ray_pred ):
        ...
        if 'gamma_clustering' in self.activation:
            with torch.no_grad():
                ...
                dist_anchors_pred, angles_num_pred_, roi_locs_pred, centerd_coords_ = try_vectorize_gamma(dc_coords_float_b.clone(), mask_logit_pred, rays_from_mask, self.rays_width, self.rays_height)
        else:
            with torch.no_grad():
