...
from .model_utils import (batch_giou_corres, batch_giou_corres_polar, giou_aabb, 
                          ...
                          try_vectorize, try_vectorize_gamma,try_vectorize_mask2ray)
import pdb
import torchvision
import numpy as np
@torch.no_grad()
...
class Criterion_Dyco_Ray_Cluster_Gamma_Ablation(nn.Module):
    def __init__(
        self,
        semantic_classes=20,
        instance_classes=18,
        ...
        super(Criterion_Dyco_Ray_Cluster_Gamma_Ablation, self).__init__()
    ...
    def get_loss_new(self, dc_coords_float_b, inst_label, ray_label, mask_feat_b, dc_proto_features_b, proto_coff_b, mask_logit_pred, ray_pred ):
        ...
        if False and (mask_logit_pred>=0).sum(1).min().item() == 0:
            ...
            with torch.no_grad():
                ...
                rays_from_mask[:,:-3] = rays_from_mask[:,:-3] + ray_pred[:,:-3] ### ori
        if 'gamma_clustering' in self.activation:
            with torch.no_grad():
                #rays_from_mask = try_vectorize_mask2ray(dc_coords_float_b.clone(),  (mask_logit_pred>=0)*1.0,  self.rays_width, self.rays_height, self.angles_ref)    
                
                
                #rays_from_mask[:,-3:] = (rays_from_mask[:,-3:] + ray_pred[:,-3:])/2.0 ### experimental
                #rays_from_mask = ray_pred
                dist_anchors_pred, angles_num_pred_, roi_locs_pred, centerd_coords_ = try_vectorize_gamma(dc_coords_float_b.clone(), mask_logit_pred, rays_from_mask, self.rays_width, self.rays_height, inst_label)
        else:
            with torch.no_grad():
        ...
        inverse_mask = -1*torch.tanh(mask_logit_pred)#for gamma clustering
        #inverse_mask = -1*torch.sigmoid(mask_logit_pred)
        #####??
        ...
        with torch.no_grad():
            ...
            FP_indice = (mask_logit_pred.reshape(-1) >= 0) * (inst_label_flatten == 0)
            FN_indice = (mask_logit_pred.reshape(-1) < 0) * (inst_label_flatten == 1)
            TP_indice = (mask_logit_pred.reshape(-1) >= 0) * (inst_label_flatten == 1)
        ...
        ####
        #FP_indice = inst_label_flatten == 0
        #FN_indice = inst_label_flatten == 1
        ####
        ...
        FP_sample_target = FP_ray_pred + 0.1#epsilon
        FN_sample_target = FN_ray_pred - 0.1#epsilon

       
        clustered_output_FP = centerd_coords_dist[FP_indice] - mask_logit_pred.reshape(-1)[FP_indice]
        clustered_output_FN = centerd_coords_dist[FN_indice] - mask_logit_pred.reshape(-1)[FN_indice]
        clustered_output_TP = centerd_coords_dist[TP_indice] - mask_logit_pred.reshape(-1)[TP_indice]
        '''
        clustered_output_FP = centerd_coords_dist[FP_indice] - torch.tanh(mask_logit_pred.reshape(-1)[FP_indice])
        clustered_output_FN = centerd_coords_dist[FN_indice] - torch.tanh(mask_logit_pred.reshape(-1)[FN_indice])
        clustered_output_TP = centerd_coords_dist[TP_indice] - torch.tanh(mask_logit_pred.reshape(-1)[TP_indice])
        '''
        if 'gamma_clustering' in self.activation:
            ...
            if len(FP_sample_target) > 0:
                ...
                #FP_input = mask_logit_pred.reshape(-1)[FP_indice]
                FP_target = torch.zeros(len(FP_input)).float().cuda()
                
                
                FP_target[:] = -1
                #FP_target[:] = 0
                #ray_FP_loss = F.soft_margin_loss(FP_input, FP_target)
                #ray_FP_loss = F.binary_cross_entropy_with_logits(FP_input, FP_target)
                if init == False:
                    inputs = FP_input
                    targets = FP_target
                    init=True
            ...
            #    FP_input = torch.zeros(1).float().cuda()
            #    FP_target = torch.zeros(1).float().cuda()
            ...
            #else:
            #    TP_input = torch.zeros(1).float().cuda()
            #    TP_target = torch.zeros(1).float().cuda()
            #    TP_target[:] = 1
            ...
            #    FN_input = torch.zeros(1).float().cuda()
            #    FN_target = torch.zeros(1).float().cuda()
            #    FN_target[:] = 1

            #ray_pred_loss = ray_FP_loss + ray_FN_loss
            
            #inputs = torch.cat([FP_input,FN_input])
            #targets = torch.cat([FP_target, FN_target])
         
            ####
            
            #if 'wo_tanh' not in self.activation:
            #    inputs = torch.tanh(inputs)
            ####
            if init:
                #ray_pred_loss = F.soft_margin_loss(inputs, targets)
                ray_pred_loss = F.soft_margin_loss(torch.tanh(inputs), targets)
            else:
                ray_pred_loss = torch.zeros(1).float().cuda()
            #ray_pred_loss = F.binary_cross_entropy_with_logits(inputs, targets)
        else:
            ray_pred_loss = torch.zeros(1).float().cuda()
            
        
        #print('FP:{0} FN:{1}'.format(FP_indice.sum(), FN_indice.sum()))
        ...
        if '2nd_refine' in self.activation:
            ...
            roi_gt_total_soft_margin = roi_gt_total.clone()
            roi_gt_total_soft_margin[roi_gt_total == 0] = -1

            ray_bce_loss = F.binary_cross_entropy_with_logits(roi_pred_total, roi_gt_total, reduction="none")
            #ray_bce_loss = F.soft_margin_loss(torch.tanh(roi_pred_total), roi_gt_total_soft_margin, reduction="none")
            #dice_loss_ray = torch.zeros(1).float().cuda()[0]
    ...
    def single_layer_loss(
        ...
        for b in range(batch_size):
            ...
            if is_mask:
                ...
                #if 'gamma_clustering' in self.activation or '2nd_refine' in self.activation:
                if 'gamma_clustering' in self.activation :
