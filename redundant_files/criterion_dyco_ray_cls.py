...
from .model_utils import batch_giou_corres, batch_giou_corres_polar, giou_aabb, giou_aabb_polar
import pdb
...
class Criterion_Dyco_Ray_CLS(nn.Module):
    def __init__(
        self,
        semantic_classes=20,
        instance_classes=18,
        ...
        super(Criterion_Dyco_Ray_CLS, self).__init__()
    ...
    def single_layer_loss(
        ...
        for b in range(batch_size):
            ...
            pred_inds, cls_label, inst_label, box_label, ray_label, ray_mask_label = row_indices[b], cls_labels[b], inst_labels[b], box_labels[b], ray_labels[b], ray_mask_labels[b]
            
            if ray_mask_label == None:
                continue
            ray_mask_label = ray_mask_label.reshape(ray_mask_label.shape[0],-1)
           
            new_inst_label = torch.zeros(inst_label.shape).float().cuda()
            new_inst_label[:,:ray_mask_label.shape[1]] = ray_mask_label
            ...
            if is_mask:
                ...
                loss_dict["dice_loss"] = loss_dict["dice_loss"] + compute_dice_loss(
                    mask_logit_pred, new_inst_label, num_gt_batch
                )
            if is_mask:
                
                #bce_loss = F.binary_cross_entropy_with_logits(mask_logit_pred, inst_label, reduction="none")
                bce_loss = F.binary_cross_entropy_with_logits(mask_logit_pred, new_inst_label, reduction="none")
            ...
            if is_mask:
                ...
                gt_iou = get_iou(mask_logit_pred, new_inst_label)
                loss_dict["iou_loss"] = (
    ...
    def forward(self, batch_inputs, model_outputs, is_mask=True):
        ...
        gt_dict, aux_gt_dict, matching_cost = self.matcher.forward_dup_cls(
            cls_logits,
            mask_logits,
            conf_logits,
            box_preds,
        ...
        ray_mask_labels = gt_dict['ray_mask_labels']
        ...
        aux_ray_mask_labels = aux_gt_dict['ray_mask_labels']
