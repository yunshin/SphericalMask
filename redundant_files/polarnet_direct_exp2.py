...
from .model_utils import (
    ...
    get_batch_offsets,
    get_instance_info,
    get_instance_info_polarnet_exp2,
    get_spp_gt,
    get_subsample_gt,
    nms,
    random_downsample,
...
import pdb
import time
class PolarNet_Direct_EXP2(nn.Module):
    def __init__(
        self,
        is_mask=True,
    ...
    def forward_train(
        ...
        **kwargs
        ...
        instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, corners_offset_labels_sph, rays_labels, spherical_mask, voxel_semantic_labels  = get_instance_info_polarnet_exp2(
    ...
    def forward_test(
        ...
        **kwargs
        ...
        if self.is_mask == False:
            ...
            if self.is_analyze:
                instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, rays_gt_list = get_instance_info_polarnet_exp2(
            ...
            instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, corners_offset_labels_sph, corners_offset_labels, spherical_mask, voxel_semantic_labels = get_instance_info_polarnet_exp2(
                voxel_coords_float, voxel_instance_labels, voxel_semantic_labels, self.rays_width, self.rays_height, self.angles_ref, label_shift=self.label_shift
            )
        ...
        voxel_batch_offsets_ = get_batch_offsets(voxel_batch_idxs_, batch_size)
        voxel_box_preds_ = torch.cat([voxel_box_preds_, voxel_coords_float_],1)
        ...
        conf_sort_indice = conf_sort_indice[:256]
       
        pos_mask = voxel_box_conf_[conf_sort_indice] > 0.15
        ...
        categories_nms, voxel_box_conf_nms, voxel_box_preds_nms, indices = standard_nms_with_iou(categories, voxel_box_conf_, voxel_box_preds_,iou_, threshold=0.3)
        
        #category_labels = voxel_semantic_labels[indices]
        category_labels = torch.argmax(voxel_semantic_scores_sm[indices],1)
        #pdb.set_trace()
        #category_labels = category_labels_pred -2
        
        #category_labels = voxel_semantic_labels[spherical_mask==1][indices]
        ...
        if self.is_debug:
            ...
            rays_label = rays_label[:,:-1]
            
            rays_label = cartesian_to_spherical(rays_label)
            rays_label = rays_label[:,:,3]
            box_in_3D = get_3D_locs_from_rays(rays_label, self.angles_ref, voxel_coords_float_[spherical_mask==1])
            box_preds_label = torch.cat([box_in_3D, voxel_coords_float_[spherical_mask==1][:,None,:]],1)
