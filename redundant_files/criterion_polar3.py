...
class Criterion_Polar3(nn.Module):
    def __init__(
        self,
        semantic_classes=20,
        instance_classes=18,
        ...
        super(Criterion_Polar3, self).__init__()
    ...
    def cal_point_wise_loss(
        ...
        if total_pos_inds == 0:
            ...
            offset_loss = (
                F.l1_loss(centroid_offset[pos_mask], spherical_center_labels_offset[pos_mask], reduction="sum")
                / pos_mask.sum()
            )
          
            anchors = self.rays_anchors_tensor[semantic_labels[pos_mask]]
            
            offset_vertices_loss = (
                F.l1_loss(corners_offset[pos_mask] + anchors, rays_labels[pos_mask], reduction="sum") / pos_mask.sum()
            )
            ...
            box_conf_sigmoid = torch.sigmoid(box_conf)
            TP_conf = box_conf_sigmoid[pos_mask] > 0.1
            #TP_conf = TP_conf.sum() / pos_mask.sum()
            FP_conf = box_conf_sigmoid[pos_mask==False] > 0.1
            #FP_conf = FP_conf.sum() / (pos_mask==False).sum()

            print('TP cls: {0}/{1} TP conf: {2}/{3} FP conf: {4}/{5}'.format(TP.sum(), len(TP), TP_conf.sum().item(), pos_mask.sum().item(),
                                                                            FP_conf.sum().item(), (pos_mask==False).sum().item()))
        ...
        losses["pw_center_loss"] = offset_loss * 2 * self.voxel_scale / 50.0
