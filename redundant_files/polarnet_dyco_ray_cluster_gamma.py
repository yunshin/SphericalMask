...
class SelfAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        #pdb.set_trace()
        return out
class PolarNet_Dyco_Ray_Cluster_Gamma(nn.Module):
    def __init__(
        self,
        model_name='polarnet_dyco_ray_cluster_gamma',
    ...
    def get_output_new(self, dc_coords_float, mask_logits, rays_labels, query_locs, dc_mask_features_permute, mask_feats, proto_coff, gt_masks, ray_preds):
        masks_proto = torch.zeros_like(mask_logits) 
        masks_proto[:,:] = -100
        ...
        dist_anchors, angles_num, roi_locs, _ = try_vectorize(dc_coords_float, (mask_logits>=0)*1.0, rays_from_mask, self.rays_width, self.rays_height)      

        #dist_anchors, angles_num, roi_locs = try_vectorize(dc_coords_float, (mask_logits>=0)*1.0, ray_corres_gt, self.rays_width, self.rays_height)      
        ...
        #2
        #roi_pred_total = torch.matmul(proto_coff, feats.T)
        #roi_pred_total = roi_pred_total[roi_locs[0],torch.arange(roi_pred_total.shape[1]).long().cuda()]

        #3
        #roi_pred_total = feats.mean(1)

        #4
        #roi_pred_total = self.inside_mask_head(feats1[:,None,:], feats2[:,None,:], feats2[:,None,:]).squeeze()#works worse than 1
        #roi_pred_total = self.inside_mask_head(feats1[:,None,:], feats1[:,None,:], feats2[:,None,:]).squeeze()#works worse than 1
    ...
    def get_output_new_ray(self, dc_coords_float, mask_logits, rays_labels, query_locs, dc_mask_features_permute, mask_feats, proto_coff, gt_masks, ray_preds):
        ...
        rays_from_mask[:,:-3] = rays_from_mask[:,:-3] + ray_preds[:,:-3] 
        #rays_from_mask[:,-3:] = rays_from_mask[:,-3:]+ ray_preds[:,-3:] 
        #rays_from_mask = ray_preds
        


        dist_anchors, angles_num, roi_locs, _ = try_vectorize_gamma(dc_coords_float, mask_logits, rays_from_mask, self.rays_width, self.rays_height)      
        ...
        **kwargs
        ...
        if vis_save:
            instance_cls_, instance_box_, instance_sphere_, centroid_offset_labels_, pointwise_angular_labels_, corners_offset_labels_, corners_offset_labels_sph_, rays_labels_, spherical_mask_, voxel_semantic_labels_, real_points  = get_instance_info_dyco_ray(
                coords_float, instance_labels, semantic_labels, self.rays_width, self.rays_height, self.angles_ref, label_shift=self.label_shift, is_vis=True
            )
