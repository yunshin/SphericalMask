...
import pdb
import numpy as np
class PolarNet_Dyco_Ray_Cluster_with_CLS(nn.Module):
    def __init__(
        self,
    ...
    def init_dyco_angular_depr(self):
        conv_block = conv_with_kaiming_uniform("BN", activation=True)
    ...
    def forward_train(
        ...
        **kwargs
        ...
        cls_logits, mask_logits, conf_logits, box_preds, rays_pred, ray_cls, mask_logit_angular = self.forward_head(
        ...
        model_outputs.update(
            dict(
                ...
                mask_logits_angular=mask_logit_angular,
    ...
    def forward_test(
        ...
        **kwargs
        ...
        for i in range(len(n_sample_arr)):
            ...
            cls_logits, mask_logits,  conf_logits, box_preds, rays_pred, ray_cls, mask_logits_angular = forward_head_outputs
            
            if type == 1:
                mask_logits = mask_logits + (mask_logits_angular) 
            mask_ray_logits = rays_pred
            ...
            if type == 0: 
                ...
                masks_ray_ = get_mask_from_polar_single2(dc_coords_float[None,:,:].repeat(len(ray_box_preds),1,1), ray_box_preds, mask_logits, self.rays_width, self.rays_height, 
                ...
                mask_logits_final.append(mask_logit)
                '''
                mask_logits_final.append(masks_ray_ori)
            elif type == 2:
    ...
    def forward_head(
        ...
        for b in range(batch_size):
            ...
            if num_points == 0:
                ...
                mask_logits_angular.append(None)
            ...
            weights_angular, biases_angular = self.parse_dynamic_params(controller, self.mask_dim_out)
            ...
            for i in range(num_chunks):
                ...
                mask_logit_angular_ = self.mask_heads_forward(
                    mask_feature_b,
                    weights_split,
                    biases_split,
                    weights_split[0].shape[0],
                ...
                mask_logit_angular.append(mask_logit_angular_)
            mask_logit = torch.cat(mask_logit, dim=0)
            mask_logit_angular = torch.cat(mask_logit_angular)

            mask_logits_angular.append(mask_logit_angular)
            mask_logits.append(mask_logit)
          
        
        if inference:
            return cls_logits[0], mask_logits[0], conf_logits[0], queries_box_preds[0], rays_pred[0], rays_cls[0], mask_logits_angular[0]
        return cls_logits, mask_logits, conf_logits, queries_box_preds, rays_pred, rays_cls, mask_logits_angular
