...
class Criterion_Polar4(nn.Module):
    def __init__(
        self,
        semantic_classes=20,
        instance_classes=18,
        ...
        super(Criterion_Polar4, self).__init__()
    ...
    def cal_point_wise_loss(
        ...
        rays_labels = spherical_ray_labels[:,:,0]
        #pos_mask = spherical_mask == 1
        

        
        semantic_loss = F.cross_entropy(semantic_scores, semantic_labels, ignore_index=self.ignore_label, weight=weight)
        ...
        if total_pos_inds == 0:
            ...
            offset_loss = (
                F.l1_loss(centroid_offset[pos_inds], spherical_center_labels_offset[pos_inds], reduction="sum")
                / total_pos_inds
            )
            
            
            label_for_anchor = semantic_labels.clone()
            label_for_anchor[label_for_anchor == -100] = 18
            anchors = self.rays_anchors_tensor[label_for_anchor[pos_inds]]
            
            rays_pred = corners_offset[pos_inds] + anchors
            offset_vertices_loss = (
                F.l1_loss(rays_pred, rays_labels[pos_inds], reduction="sum") / pos_inds.sum()
            )
            ...
            if len(angular_offset_labels) > 1 :
                ...
                angular_offset_loss = angular_offset_loss.sum() / total_pos_inds#total_pos_inds
            ...
            box_pred = torch.cat([rays_pred,centroid_offset[pos_inds]],1)
            box_label = torch.cat([rays_labels[pos_inds], spherical_center_labels_offset[pos_inds]],1)
            ...
            conf_loss = F.l1_loss(box_conf[pos_inds], iou_gt, reduction="sum") / total_pos_inds

        losses["pw_center_loss"] = offset_loss * 2 * self.voxel_scale / 15.0
        losses["pw_corners_loss"] = offset_vertices_loss * self.voxel_scale / 15.0
        #losses["pw_corners_loss"] = torch.zeros(1).float().cuda()
        losses["pw_angular_loss"] = angular_offset_loss * 0.1
        losses["pw_giou_loss"] = torch.zeros(1).float().cuda()
        losses["pw_conf_loss"] = conf_loss * 2
