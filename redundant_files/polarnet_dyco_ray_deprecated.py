...
class PolarNet_Dyco_Ray(nn.Module):
    def __init__(
        self,
        model_name='polarnet_dyco_ray',
        ...
        #self.angular_linear = MLP(channels, 2, norm_fn=norm_fn, num_layers=2)
        ...
        if not self.semantic_only:
            ...
            self.point_aggregator1_2 = LocalAggregator(
                mlp_dim=self.channels,
                n_sample=instance_head_cfg.n_sample_pa1,
                radius=0.2 * instance_head_cfg.radius_scale,
                n_neighbor=instance_head_cfg.neighbor,
            ...
            self.point_aggregator2_2 = LocalAggregator(
                mlp_dim=self.channels * 2,
                n_sample=2,
                radius=0.4 * instance_head_cfg.radius_scale,
                n_neighbor=instance_head_cfg.neighbor,
            ...
            self.inst_angular_head = GenericMLP(
            ...
            self.mask_angular_head = GenericMLP(
                input_dim=32,
                hidden_dims=[32, 32],
                norm_fn_name="bn1d",
                activation="relu",
                use_conv=True,
                output_dim=2,  # xyz_min, xyz_max
            )
    ...
    def init_dyco_angular(self):
        ...
        weight_nums = [(self.mask_dim_out + 3 + 3) * self.mask_dim_out, self.mask_dim_out * 2]
        bias_nums = [self.mask_dim_out, 2]
    ...
    def forward_train(
        ...
        **kwargs
        ...
        query_locs1_2, query_feats1_2, query_boxes1_2, query_inds1 = self.point_aggregator1_2(
            voxel_coords_float_,
            voxel_output_feats_,
            voxel_box_preds_,
            batch_offsets=voxel_batch_offsets_,
        ...
        query_locs2, query_feats2, query_boxes2, query_inds2 = self.point_aggregator2_2(
            query_locs1_2, query_feats1_2, query_boxes1_2, batch_offsets=None, batch_size=batch_size, sampled_before=True
        )
        ...
        if self.use_spp_pool:
            ...
            dc_inst_mask_arr, unique_voxels_locs_gt = get_spp_gt_rays2(
        ...
        mask_logits_angular = self.forward_head_angular(
            query_feats, query_locs, dc_mask_features, dc_coords_float, dc_box_preds, dc_batch_offsets
        )
        

        cls_logits, mask_ray_logits, conf_logits, box_preds = self.forward_head_ray(
            query_feats, query_locs, dc_mask_features_ray, dc_coords_float, dc_box_preds, dc_batch_offsets
        )
        ...
        model_outputs.update(
            dict(
                ...
                mask_logits=mask_logits_angular,
    ...
    def forward_test(
        ...
        **kwargs
        ...
        angular_gt = dc_inst_mask_arr[0]['angular_gt']
        rays_gt = dc_inst_mask_arr[0]['rays']
        ...
        for i in range(len(n_sample_arr)):
            ...
            query_locs2_angular, query_feats2_angular, query_boxes2_angular, query_inds2_angular = self.point_aggregator2_2(
            query_locs2, query_feats2, query_boxes2, batch_size=batch_size, sampled_before=False
            )
            
            forward_head_outputs_angular = self.forward_head_angular(
                query_feats2_angular,
                query_locs2_angular,
                dc_mask_features,
                dc_coords_float,
                dc_box_preds,
            ...
            forward_head_outputs_ray = self.forward_head_ray(
                query_feats,
                query_locs,
            ...
            if forward_head_outputs_ray is None:
                break
            mask_logits = forward_head_outputs_angular
            cls_logits, mask_ray_logits, conf_logits, box_preds = forward_head_outputs_ray
            ...
            #np.save('/root/data/tmp/angular_gt.npy', angular_gt.cpu().numpy())
            #pdb.set_trace()
            if self.is_debug == False:
                ...
                masks_ray_, masks_angles_ = get_mask_from_polar_single(dc_coords_float, ray_box_preds, self.rays_width, self.rays_height, 
                                                                    is_angular=True, pointwise_angular=mask_logits.squeeze(),#mask_logits,#angular_pred_list[0],#
                                                                        sim_ths = 0.80 )
                ...
                rays_gt = rays_gt[:,:,0]
                box_in_3D = get_3D_locs_from_rays(rays_gt[:,:-3], self.angles_ref, rays_gt[:,-3:])
               

               
                ray_box_preds = torch.cat([box_in_3D, rays_gt[:,None,-3:]],1)
                ray_box_preds = ray_box_preds.reshape(len(ray_box_preds),-1)
                masks_ray_, masks_angles_ = get_mask_from_polar_single(dc_coords_float, ray_box_preds, self.rays_width, self.rays_height, 
                                                                    is_angular=True, pointwise_angular=angular_gt,
                                                                        sim_ths = 0.95, gt_locs=unique_voxels_locs_gt )
                
                instance_cls = instance_cls[instance_cls!=-100] 
                gt_array = torch.cat([torch.arange(len(instance_cls)).long().cuda()[:,None], instance_cls[:,None]], 1)
                
                cls_logits_gt = torch.zeros(len(instance_cls), 19).float().cuda()
                cls_logits_gt[:,:] = -5
                
                cls_logits_gt[gt_array[:,0], gt_array[:,1]] = 5
                cls_logits_final.append(cls_logits_gt)
                #mask_logits_final.append(mask_logit)
                conf_logits = torch.ones(len(instance_cls)).float().cuda()
                conf_logits_final.append(conf_logits)
                box_preds_final.append(box_preds[:len(instance_cls)])
            masks_ray_ = masks_ray_[:, voxel_spps]
            masks_angles_ = masks_angles_[:, voxel_spps]
            mask_rays_final.append(masks_ray_)
            mask_angles_final.append(masks_angles_)
            ##################################
    ...
    def forward_pointwise_head(self, output_feats):
        ...
        with context():
            ...
            #angular_offset = self.angular_linear(output_feats)
    ...
    def forward_head_angular(
        ...
        if inference:
            ...
            queries_features_ori = queries_features.clone()
            '''
        ...
        n_queries_ori = queries_features_ori.shape[2]

        controllers = (
            self.inst_mask_head(queries_features_ori.permute(0, 2, 1)[..., None].flatten(0, 1))
            .squeeze(dim=2)
            .reshape(batch_size, n_queries_ori, -1)
        )

        mask_logits = []
        
        closest_angles_indice = []
        point_mean_dist_list = []
        for b in range(batch_size):
            ...
            weights, biases = self.parse_dynamic_params_angular(controller, self.mask_dim_out)
            ...
            if not self.training and self.dataset_name == "s3dis":
                ...
                chunk_size = n_queries_ori
            
            mask_logit = []
            mask_ray_logit = []
            closest_angles_indice_b = []
            point_mean_dist_list_b = []
            for i in range(num_chunks):
                ...
                mask_logit_ = mask_logit_.mean(0)
    ...
    def forward_head_ray(
        self, queries_features, queries_locs, mask_ray_features_, locs_float_, box_preds_, batch_offsets_, inference=False
    ):
        ...
        controllers_ray = (
            self.inst_mask_ray_head(queries_features_ori.permute(0, 2, 1)[..., None].flatten(0, 1))
            .squeeze(dim=2)
            .reshape(batch_size, n_queries_ori, -1)
        )
        ...
        if inference:
            ...
            return cls_logits[0][fg_conds], mask_ray_logits[0][fg_conds], conf_logits[0][fg_conds], queries_box_preds[0][fg_conds]
        return cls_logits, mask_ray_logits, conf_logits, queries_box_preds
    ...
    def parse_dynamic_params_angular(self, params, out_channels):
        ...
        weight_splits[1] = weight_splits[1].reshape(num_instances, out_channels, 2)###Different number of out channel 
        #weight_splits[1] = weight_splits[1].reshape(2, num_instances, out_channels)
        bias_splits[1] = bias_splits[1].reshape(num_instances, 2)
    ...
    def mask_heads_forward_tmp(
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):
            ...
            if i < n_layers - 1:
                ...
                #x = torch.einsum("qab,qan->qbn", w, x) +b.unsqueeze(-1)
                x = torch.einsum("qab,naq->nqb", w, x)# +b.unsqueeze(-1)
                x = F.relu(x) #256X32X1618
            else:
                #pdb.set_trace()
                #we = w.permute(2, 0, 1)#2 256 32
                #x = x.permute(2,0, 1)#1618 256 32
                
                #x = torch.einsum("abc,dbe->dab", w, x)
                
                x = torch.einsum("qab,nqa->nab", w, x)
                #x = torch.einsum("qab,qan->qnb", w, x) # NOTE Empirically, we do not add biases in last dynamic_conv layer
                #x = x.squeeze(1)
