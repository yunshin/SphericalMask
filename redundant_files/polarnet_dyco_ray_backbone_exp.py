...
class PolarNet_Dyco_Ray(nn.Module):
    def __init__(
        ...
        if not self.semantic_only:
            ...
            self.inst_angular_head = GenericMLP(
                ...
                activation="relu",
                use_conv=True,
                output_dim=128,  # xyz_min, xyz_max
            )
    ...
    def init_dyco_angular(self):
        ...
        self.add_module("mask_tower", nn.Sequential(*mask_tower))
        '''
        weight_nums_angular = [(self.mask_dim_out_angular + 3 + 3  ) * self.mask_dim_out_angular, self.mask_dim_out_angular * 2]
        bias_nums_angular = [self.mask_dim_out_angular, 2]
        ...
        controller_head = nn.Conv1d(128, self.num_gen_params_angular, kernel_size=1)
        torch.nn.init.normal_(controller_head.weight, std=0.01)
        torch.nn.init.constant_(controller_head.bias, 0)
        inst_mask_head.append(controller_head)
    ...
    def forward_train(
        ...
        **kwargs
        ...
        if self.use_spp_pool:
            ...
            dc_inst_mask_arr = get_spp_gt_rays(
        ...
        cls_logits, mask_logits, mask_ray_logits, conf_logits, box_preds, angular_pred = self.forward_head(
    ...
    def forward_test(
        ...
        **kwargs
        ...
        for i in range(len(n_sample_arr)):
            ...
            cls_logits, mask_logits, mask_ray_logits, conf_logits, box_preds, angular_pred = forward_head_outputs
            ...
            masks_ray_, masks_angles_ = get_mask_from_polar_single(dc_coords_float, ray_box_preds, self.rays_width, self.rays_height, 
                                                                   is_angular=True, pointwise_angular=angular_pred,
                                                                    sim_ths = 0.8 )
    ...
    def forward_head(
        ...
        controllers_angular = (
            self.inst_mask_head_angular(mask_features_)
            .squeeze(dim=2)
            #.reshape(batch_size, n_queries, -1)
        )
        ...
        for b in range(batch_size):
            ...
            controller_angular = controllers_angular[start:end]
            ...
            for i in range(num_chunks):
                ...
                weights_angular_split = weights_angular
                biases_angular_split =  biases_angular
        ...
        if inference:
            return cls_logits[0], mask_logits[0], mask_ray_logits[0], conf_logits[0], queries_box_preds[0], angular_preds[0]
        return cls_logits, mask_logits, mask_ray_logits, conf_logits, queries_box_preds, angular_preds
    ...
    def parse_dynamic_params_angular(self, params, out_channels):
        ...
        weight_splits[0] = weight_splits[0].reshape(num_instances, out_channels , out_channels)
        bias_splits[0] = bias_splits[0].reshape(num_instances, out_channels)
        weight_splits[1] = weight_splits[1].reshape(num_instances, out_channels, 2)
    ...
    def angular_heads_forward(
        ...
        relative_coords = relative_coords.mean(0)
       
        #x = torch.cat([angular_features, coords_[:,:,None]],1).repeat(1,1,32)
        x = angular_features.repeat(1,1,128)
      
        #x = torch.cat([angular_features, relative_coords[:,:,None]],1).repeat(1,1,128)
        #x = angular_features.permute(1,0,2)#.repeat(1,128,1)
        
        #coords
        #weights[0] = weights[0].permute(2,0,1)
        #weights[1] = weights[1].permute(1,)
        ###
        '''
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):
            if i < n_layers - 1:
                ...
                x = torch.einsum("qab,qan->qbn", x, w) + b.unsqueeze(-1)
                x = F.relu(x) #256X32X1618
            else:
                
                x = torch.einsum("qab,qan->qbn", x, w) # NOTE Empirically, we do not add biases in last dynamic_conv layer
                x = x.squeeze(1)
