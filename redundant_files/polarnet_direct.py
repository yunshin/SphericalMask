...
import pdb
import time
class PolarNet_Direct(nn.Module):
    def __init__(
        self,
        is_mask=True,
        model_name='polarnet_direct',
        channels=32,
        num_blocks=7,
        semantic_only=False,
        semantic_classes=20,
        ...
        self.offset_vertices_linear_x = MLP(channels, self.rays_width*self.rays_height +1, norm_fn=norm_fn, num_layers=2)
        self.offset_vertices_linear_y = MLP(channels, self.rays_width*self.rays_height +1, norm_fn=norm_fn, num_layers=2)
        self.offset_vertices_linear_z = MLP(channels, self.rays_width*self.rays_height +1, norm_fn=norm_fn, num_layers=2)
        ...
        if not self.semantic_only:
            ...
            if rays_width == 0:
                ...
                self.inst_box_head_x = GenericMLP(
                    input_dim=instance_head_cfg.dec_dim,
                    hidden_dims=[instance_head_cfg.dec_dim, instance_head_cfg.dec_dim],
                    norm_fn_name="bn1d",
                    activation="relu",
                    use_conv=True,
                    output_dim=rays_width*rays_height + 1,  # xyz_min, xyz_max
                )
                self.inst_box_head_y = GenericMLP(
                    input_dim=instance_head_cfg.dec_dim,
                    hidden_dims=[instance_head_cfg.dec_dim, instance_head_cfg.dec_dim],
                    norm_fn_name="bn1d",
                    activation="relu",
                ...
                self.inst_box_head_z = GenericMLP(
                    input_dim=instance_head_cfg.dec_dim,
                    hidden_dims=[instance_head_cfg.dec_dim, instance_head_cfg.dec_dim],
                    norm_fn_name="bn1d",
                    activation="relu",
    ...
    def forward_train(
        ...
        **kwargs
        ...
        instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels = get_instance_info_polarnet(
        ...
        # -------------------------------
        '''
        #masks = get_mask_from_polar(torch.cat([voxel_batch_idxs[:,None],voxel_coords_float],1), box_preds, self.rays_width, self.rays_height)
        
        masks_gt, masks_angles_gt = get_mask_from_polar(torch.cat([voxel_batch_idxs[:,None],voxel_coords_float],1), instance_box[None,:], self.rays_width, self.rays_height,
                                        is_angular=True, pointwise_angular=pointwise_angular_labels )
        
        
        get_mask_iou(voxel_batch_idxs, masks_angles_gt, voxel_instance_labels)
        #get_mask_iou(voxel_batch_idxs, masks_gt, voxel_instance_labels)
        pdb.set_trace()
        '''
        ...
        with torch.no_grad():
            ...
            val,cnt = 0,0
            for batch_val in range(len(box_preds)):

                box_preds_batch = box_preds[batch_val]
               
                if dc_inst_mask_arr[batch_val] == None:
                    continue
                if 'box' not in dc_inst_mask_arr[batch_val]:
                    continue
                if dc_inst_mask_arr[batch_val]['box'] == None:
                    continue
                box_labels = dc_inst_mask_arr[batch_val]['box']
                
                ious_batch,_ = batch_giou_cross_polar(box_labels, box_preds_batch)
                ious_batch,_ = torch.max(ious_batch,1)
            
                if len(ious_batch) > 0:
                    val += torch.sum(ious_batch)
                    cnt += len(ious_batch)
            print('IoUs mean : {0} '.format(val/(cnt+1e-6)))
        if self.is_mask:
            model_outputs.update(
                dict(
    ...
    def forward_test(
        ...
        **kwargs
        ...
        if self.is_mask == False:
            ...
            if self.is_analyze:
                instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, rays_gt_list = get_instance_info_polarnet(
                ...
                return instance_cls, instance_box, instance_sphere, centroid_offset_labels, pointwise_angular_labels, corners_offset_labels, rays_gt_list
        ...
        for i in range(len(n_sample_arr)):
            ...
            if self.is_debug:
                ...
                #labels_form = labels_form[labels_mask]
                #conf_logits = conf_logits[labels_mask]
                #cls_logits = cls_logits[labels_mask]
                #box_preds = box_preds[labels_mask]
                cls_logits[:]= -3
                ...
                '''
                dist_with_gt = torch.cdist(box_preds, instance_sphere.reshape(instance_sphere.shape[0],-1))
                _, best_indice = torch.min(dist_with_gt,1)
                box_preds = instance_sphere[best_indice]
                try:
            ...
            pdb.set_trace()
            masks_ray_, masks_angles_ = get_mask_from_polar_single(voxel_coords_float, box_preds, self.rays_width, self.rays_height, is_angular=True, pointwise_angular=voxel_angular_offset )
    ...
    def forward_pointwise_head(self, output_feats):
        ...
        with context():
            ...
            corners_offset_x = self.offset_vertices_linear_x(output_feats)
            corners_offset_y = self.offset_vertices_linear_y(output_feats)
            corners_offset_z = self.offset_vertices_linear_z(output_feats)
            corners_offset = torch.cat([corners_offset_x[:,:,None], corners_offset_y[:,:,None], corners_offset_z[:,:,None]],2)
    ...
    def forward_head(
        ...
        box_offsets_preds_x = self.inst_box_head_x(queries_features).transpose(1, 2)
        box_offsets_preds_y = self.inst_box_head_y(queries_features).transpose(1, 2)
        box_offsets_preds_z = self.inst_box_head_z(queries_features).transpose(1, 2)
        box_offsets_preds = torch.cat([box_offsets_preds_x[:,:,:,None], box_offsets_preds_y[:,:,:,None], box_offsets_preds_z[:,:,:,None]],3)
        #box_offsets_preds = box_offsets_preds.view(box_offsets_preds.shape[0],box_offsets_preds.shape[1], box_offsets_preds.shape[2]*box_offsets_preds.shape[3])
        
        
        #queries_box_preds = box_offsets_preds + queries_locs.repeat(1, 1, 2)
        
        queries_box_preds = box_offsets_preds + queries_locs[:,:,None,:].repeat(1, 1, (self.rays_width*self.rays_height)+1, 1)
