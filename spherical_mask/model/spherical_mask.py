...
class SphericalMask(nn.Module):
    ...
    def mask_assembly(self, dc_coords_float, rpm_output, ray_preds, rays_from_mask=False):
        
        mask_placeholder = torch.zeros_like(rpm_output) 
        mask_placeholder[:,:] = -100
        
        if rays_from_mask:
            rays_from_mask = mask2ray(dc_coords_float,  (mask_placeholder>=0)*1.0,  self.rays_width, self.rays_height, self.angles_ref)    
            ray_preds[ray_preds<0] = 0
            rays_from_mask[:,:-3] = rays_from_mask[:,:-3] + ray_preds[:,:-3]
        else: 
            ray_preds[ray_preds<0.01] = 0.01
            rays_from_mask = ray_preds

        _, angles_num_pred_, _, centerd_coords_ = find_sector(dc_coords_float, rpm_output, rays_from_mask, self.rays_width, self.rays_height, point_migration=True)      
        ...
        migrated_points = centerd_coords_dist - rpm_output.reshape(-1)
        final_mask = ray_dist_with_angles - migrated_points
        
        final_mask = (final_mask >= 0)*1.0
        final_mask = final_mask.reshape(rpm_output.shape)
        
        
        masks_proto = final_mask
    ...
    def forward_train(
        ...
        **kwargs
        ...
        if self.use_spp_pool:
            ...
            dc_inst_mask_arr = get_spp_gt_ray(
    ...
    def forward_test(
        ...
        **kwargs
        ...
        for i in range(len(n_sample_arr)):
            ...
            cls_logits, rpm_output, conf_logits, box_preds, _, _, ray_preds = forward_head_outputs
        
            masks_ = self.mask_assembly(dc_coords_float, rpm_output, ray_preds)
            mask_logit = masks_[:, voxel_spps]
            ...
            mask_logits_0 = (rpm_output[:, voxel_spps_][:, query_inds1[0].long()].detach() > 0).float()
    ...
    def forward_head(
        ...
        ray_offsets_preds = self.rid_module(queries_features).transpose(1, 2)
    ...
    def mask_heads_forward(
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):
            if i < n_layers - 1:
                ...
                x = torch.einsum("qab,qan->qbn", w, x) # NOTE Empirically, we do not add biases in last dynamic_conv layer, following ISBNet.
                x = x.squeeze(1)
