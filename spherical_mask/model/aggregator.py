...
from spherical_mask.ops import ballquery_batchflat, furthestsampling_batchflat
from spherical_mask.pointnet2.pointnet2_utils import ball_query, furthest_point_sample
...
class LocalAggregator(nn.Module):
    def __init__(
        ...
        if num_rays_width == 0: 
            ...
            mlp_spec1[0] += num_rays_width*(num_rays_height) + 3  +3# +3 for center offset
            
        self.mlp_module1 = SharedMLP(mlp_spec1, bn=bn)

        mlp_spec2 = [mlp_dim * 2, mlp_dim * 2]
        if use_xyz and len(mlp_spec2) > 0 and num_rays_width == 0:
            mlp_spec2[0] += 3 + 3
        else:
           
            mlp_spec2[0] += num_rays_width*(num_rays_height) + 3 + 3
    ...
    def forward_batchflat(self, locs, feats, boxes, batch_offsets, batch_size, sampled_before=False):
        ...
        fps_offsets = torch.arange(0, self.n_sample * (batch_size + 1), self.n_sample, dtype=torch.int, device=locs.device )
        fps_inds = furthestsampling_batchflat(locs, batch_offsets, fps_offsets)
        fps_inds[fps_inds >= locs.shape[0]] = locs.shape[0] -1
