...
from spherical_mask.pointnet2.pointnet2_utils import FurthestPointSampling_
...
class CustomDataset(Dataset):
    ...
    def __init__(self, data_root, prefix, suffix, voxel_cfg=None, training=True, repeat=1, logger=None, debug=False):
        ...
        self.coi = []
        self.fps = FurthestPointSampling_()
    ...
    def get_filenames(self):
        ...
        #filenames = filenames*self.repeat
        return filenames
    ...
    def dataAugment(self, xyz, jitter=False, flip=False, rot=False, prob=1.0):
        ...
        if jitter and np.random.rand() < prob:
            #m += np.random.randn(3, 3) * 0.005
            m += np.random.randn(3, 3) * 0.1 #ori
            #m += np.random.randn(3, 3) * 0.3
            #m += np.random.randn(3, 3) * 0.5

        if rot and np.random.rand() < prob:
           
            theta = np.random.rand() * 2 * math.pi
            #theta = np.random.rand() * 1.5 * math.pi
            m = np.matmul(
    ...
    def transform_train(self, xyz, rgb, semantic_label, instance_label, spp, aug_prob=1.0):
        ...
        if np.random.rand() < aug_prob:
            xyz = self.elastic(xyz, 10, 60.0) 
            xyz = self.elastic(xyz, 30, 180.0)
    ...
    def __getitem__(self, index):
        ...
        if len(self.coi) > 0:
            ...
            semantic_label_num = int(semantic_label.max())
            instance_label_num = int(instance_label.max())
            for idx in range(semantic_label_num+1):
                
                if idx not in self.coi:
                    points_mask = semantic_label == idx
                    
                    semantic_label[points_mask] = -100
                    instance_label[points_mask]= -100
